    ...
    (function() {
      ...
      Tablesort2.prototype = {
        ...
        sortTable: function(header2, update) {
          var that = this, columnKey = header2.getAttribute("data-sort-column-key"), column = header2.cellIndex, sortFunction = caseInsensitiveSort, item = "", items = [], i = that.thead ? 0 : 1, sortMethod = header2.getAttribute("data-sort-method"), sortRevers = header2.hasAttribute("data-sort-reverse"), sortOrder = header2.getAttribute("aria-sort");
...
var isEmbeddedLicensePage = (url2) => {
  ...
  return path[1] === "embed" && typeof lastPath === "string" && lastPath && !lastPath.includes("index.html");
};
var isLicense = (url2) => {
...
async function locateMissingWorker() {
  ...
  if (response.ok) {
    ...
    const cacheWorker = meta.find(([key, _value]) => key.match(/^docs\/cacheWorker.js$/));
    if (cacheWorker) {
...
var cacheWorkerUrl = "cacheWorker.js";
if ("serviceWorker" in navigator && window.isSecureContext) {
  logger.debug("Registering service worker");
  const register2 = async () => {
    await navigator.serviceWorker.register(cacheWorkerUrl, { scope: "/" }).catch(async (error) => {
  ...
  ScrollTrigger4.enable = function enable() {
    ...
    if (gsap4) {
      ...
      if (_body3) {
        ...
        var bodyHasStyle = _body3.hasAttribute("style"), bodyStyle = _body3.style, border = bodyStyle.borderTopStyle, AnimationProto = gsap4.core.Animation.prototype, bounds, i;
...
ScrollTrigger3.batch = function(targets, vars) {
  var result = [], varsCopy = {}, interval = vars.interval || 0.016, batchMax = vars.batchMax || 1e9, proxyCallback = function proxyCallback2(type, callback) {
    var elements = [], triggers = [], delay2 = gsap4.delayedCall(interval, function() {
...
var isPageVisible$ = fromEvent(document, "visibilitychange").pipe(map(() => !document.hidden || document.visibilityState === "visible"), debounceTime(200), distinctUntilChanged((prev, curr) => prev === curr), startWith(document.visibilityState === "visible"), shareReplay(1));
...
function isPartiallyInViewport(el) {
  return watchViewportAt(el, { header$: header$2, viewport$: viewport$3 }).pipe(map(({ offset: { y }, size: { height } }) => {
    const elementHeight = el.offsetHeight;
    return y < height && y + elementHeight > 0;
  }), distinctUntilChanged((prev, curr) => prev === curr), shareReplay(1));
}
var mapLocation = (ev) => {
  const eventMap = {
...
function watchLicenseHash() {
  ...
  ]).pipe(filter(([first2, second]) => first2.pathname === second.pathname), map(([_, second]) => second), filter((url2) => Boolean(isLicenseHash(url2))), tap(handleLicenseHash));
}
...
function parsePath(path) {
  ...
  const ext = nameParts.length > 1 ? (_a2 = nameParts.pop()) != null ? _a2 : "" : "";
...
function fixSvgDimensions() {
  ...
  requestAnimationFrame(() => {
    valueMap.forEach((item) => {
      ...
      svg == null ? void 0 : svg.setAttribute("width", item.value);
      svg == null ? void 0 : svg.setAttribute("height", item.value);
    });
  });
}
...
var HeroStore = class _HeroStore {
  ...
  constructor() {
    ...
    this.parallaxHeight$ = new BehaviorSubject(Number.parseFloat((getViewportOffset().y * 1.4).toFixed(2)));
   ...
   */
  initSubscriptions() {
    const atHome$ = navigationEvents$.pipe(map(isHome), distinctUntilChanged((a, b) => a === b), startWith(isHome(initialUrl)), shareReplay(1), tap(this.createObserver("atHome$", (atHome) => ({ atHome }))));
    ...
    }), distinctUntilChanged((a, b) => a === b), shareReplay(1));
    const landingVisible$ = atHome$.pipe(filter((atHome) => atHome), switchMap(() => landing$), filter((landingVisible) => !!landingVisible), tap(this.createObserver("landingVisible$", (landingVisible) => ({
      landingVisible
    }))));
    const pageVisible$ = isPageVisible$.pipe(tap(this.createObserver("pageVisible$", (pageVisible) => ({ pageVisible }))));
    const motion$ = prefersReducedMotion$.pipe(tap(
      // biome-ignore lint/nursery/noSecrets: yeah... no.
      this.createObserver("prefersReducedMotion$", (prefersReducedMotion) => ({
        prefersReducedMotion
      }))
    ));
    const view$ = viewport$4.pipe(distinctUntilChanged((a, b) => a.offset.x === b.offset.x && a.offset.y === b.offset.y), debounceTime(100), shareReplay(1), tap((viewport) => {
    ...
    const tab$ = watchTabs(getComponentElement("tabs"), { header$: header$3, viewport$: viewport$4 });
    ...
    }), distinctUntilChanged((a, b) => a === b), shareReplay(1), tap((parallaxHeight) => {
      setCssVariable("--parallax-height", "".concat(parallaxHeight, "px"));
    }), tap(this.createObserver("parallaxHeight$", (parallaxHeight) => ({
      parallaxHeight
    }))));
  ...
  debugStateChange(updates) {
    if (weAreDev) {
      ...
      for (const [name, predicate] of Object.entries(predicates).filter(([_, value]) => typeof value === "function")) {
...
gsapWithCSS.registerEffect({
  ...
  effect: (targets, config6) => {
    ...
    return gsapWithCSS.timeline({ extendTimeline: true, paused: false }).add(gsapWithCSS.set(targets, { zIndex: 0, ...hide() })).add(gsapWithCSS.to(section.bg, { opacity: 0, yPercent: -15 * dFactor, zIndex: 0 })).add(gsapWithCSS.set([section.outerWrapper, section.innerWrapper], {
      zIndex: -1,
      ...show()
...
var fade = (targets, config6 = {
  ...
  if (fromConfig && toConfig) {
    fadeVars = getFadeVars(!!out, Number.parseInt(((_a2 = fromConfig.yPercent) == null ? void 0 : _a2.toString()) || "50", 10) || Number.parseInt(((_b = toConfig == null ? void 0 : toConfig.yPercent) == null ? void 0 : _b.toString()) || "0", 10) || 50, direction || 1);
  } else {
    fadeVars = getFadeVars(!!out, Number.parseInt(((_c = OBSERVER_CONFIG.fades.fadeInConfig.normal.from.yPercent) == null ? void 0 : _c.toString()) || "50", 10), direction || 1);
  }
...
gsapWithCSS.registerEffect({
  ...
  effect: (targets, config6) => {
    ...
    return gsapWithCSS.timeline({ extendTimeline: true }).set(targets, { autoAlpha: 1, z: 1, zIndex: 1 }).fromTo([section.outerWrapper, section.innerWrapper], {
      yPercent: (i) => i ? i * -100 * dFactor : 100 * dFactor
    }, {
      yPercent: 0,
      zIndex: -500
    }, 0).fromTo(section.bg, { yPercent: 15 * dFactor }, { opacity: 1, yPercent: 0, zIndex: -300 }, 0).add([
      "fadeIn",
      fade(fadeInTargets, {
    ...
    ], ">").add(gsapWithCSS.to(section.content, { autoAlpha: 1, zIndex: 1 }), "fadeIn+=0.3").add(revealHeader, ">");
  },
  extendTimeline: true,
...
function getContainerTarget(targets) {
  return (Array.isArray(targets) ? targets[0] : targets) || document.querySelector(".text-animation__container");
}
function getContainers(fragDivs, innerContainer, containerTarget) {
  return [
    ...fragDivs,
    innerContainer,
...
function setupLetterDivs(fragDivs, _fromVars, _toVars, _totalDuration) {
...
function animateLettersInWords(fragDivs, fromVars, toVars, totalDuration) {
  for (const div of fragDivs) {
    if (div instanceof HTMLDivElement) {
      logger.debug("Animating letters in word");
      gsapWithCSS.set(div, { display: "flex", visibility: "visible" });
      const letters = gsapWithCSS.utils.toArray(".hero__letter", div);
...
function validateAnimateTextInputs(scope, targets, config6) {
  var _a2;
  ...
  if (!(containerTarget && message)) {
    ...
    return { containerTarget: null, message: void 0 };
  }
  return { containerTarget, message };
}
function prepareMessageFragment(message, containerTarget) {
  ...
  if (!(innerContainer && containerTarget && msgFrag)) {
    ...
    return { fragDivs: [], innerContainer: null, msgFrag: null };
  }
...
function applySharedVars(config6) {
...
function configureReducedMotion(messageTimeline, fromVars, toVars) {
  gsapWithCSS.matchMedia().add({ reducedMotion: "(prefers-reduced-motion: reduce)" }, (context4) => {
    const { reducedMotion } = context4.conditions;
    if (reducedMotion) {
...
function animateText(scope, targets, config6) {
  const { message, containerTarget } = validateAnimateTextInputs(scope, targets, config6);
  if (!(containerTarget && message)) {
    return gsapWithCSS.timeline();
  }
...
gsapWithCSS.registerEffect({
  effect: (targets, config6) => {
    var _a2, _b, _c;
    const scope = ((_a2 = config6.sharedVars) == null ? void 0 : _a2.callbackScope) || config6.callbackScope || // biome-ignore lint/complexity/useLiteralKeys: This clarifies that these are not standard API properties
    ((_b = config6.sharedVars) == null ? void 0 : _b["defaults"]["callbackScope"]) || ((_c = config6.toVars) == null ? void 0 : _c.callbackScope);
...
var HeroObservation = class _HeroObservation {
   ...
   */
  static getInstance() {
    if (!_HeroObservation.instance) {
      _HeroObservation.instance = new _HeroObservation();
    }
    return _HeroObservation.instance;
  }
  ...
  setupSubscriptions() {
    ...
    const index$2 = this.indexSubject.asObservable().pipe(distinctUntilChanged((a, b) => a === b), skipUntil(atHome$), distinctUntilChanged((a, b) => a === b));
    const tearDown$ = this.store.state$.pipe(map((state) => state.atHome), distinctUntilChanged((a, b) => a === b), filter((atHome) => !atHome), debounceTime(5e3));
    this.subscriptions.add(atHome$.subscribe(() => {
      this.onLoad();
  ...
  setupSections() {
    ...
    this.sections = sectionEls.map((el, index) => {
      ...
      const initialContent = getContentElements(el).filter((element) => element !== outerWrapper && element !== innerWrapper && element !== bg).filter((e) => {
        var _a2, _b, _c, _d;
        const parent = e.parentElement || (((_a2 = this.sections[index]) == null ? void 0 : _a2.bg) instanceof Element ? (_b = this.sections[index]) == null ? void 0 : _b.bg : void 0) || (((_c = this.sections[index]) == null ? void 0 : _c.element) instanceof Element ? (_d = this.sections[index]) == null ? void 0 : _d.element : void 0);
        if (!parent) {
          return false;
        }
      ...
      logger.debug("Setting up section ".concat(index), { bg, content, innerWrapper, outerWrapper });
   ...
   * @returns
   */
  transition(direction, scenicRoute) {
    if (this.animating) {
      return;
    }
    this.animating = true;
    ...
    if (!scenicRoute) {
      ...
      this.goToSection(nextIndex, direction);
      const autoAdvance = (currentIdx, remaining) => {
        if (remaining > 0) {
          setTimeout(() => {
            if (this.currentIndex !== this.sectionIndexLength && this.currentIndex === currentIdx) {
              this.goToSection(currentIdx + 1 /* Down */, direction);
  ...
  constructTransitionTimeline(direction, index, tl) {
    ...
    if (!section) {
      logger.error("Section at index ".concat(index, " is undefined. Aborting transition."));
      this.animating = false;
      return tl;
    }
  ...
  goToSection(index, direction) {
    ...
    if (!((_a2 = this.sections[clampedIndex]) == null ? void 0 : _a2.animation)) {
  ...
  onActionFunction(direction, scenicRoute = false) {
    return () => {
      if (this.animating) {
        ...
        logger.info("Action function triggered - direction: ".concat(direction, ", scenicRoute: ").concat(scenicRoute));
      }
    };
  }
...
import svgLogo from "../images/logo_only_color_transp.WJCCEJU7.svg";
import breakFreeAv11280 from "../videos/hero/break_free/break_free_av1_1280.RSEG4CUZ.webm";
import breakFreeAv11920 from "../videos/hero/break_free/break_free_av1_1920.N4GUZFCE.webm";
import breakFreeAv12560 from "../videos/hero/break_free/break_free_av1_2560.MIKJMNDK.webm";
import breakFreeAv13840 from "../videos/hero/break_free/break_free_av1_3840.TPJHXYML.webm";
import breakFreeAv1426 from "../videos/hero/break_free/break_free_av1_426.J3IUQ25H.webm";
import breakFreeAv1640 from "../videos/hero/break_free/break_free_av1_640.KCOIXYDS.webm";
import breakFreeAv1854 from "../videos/hero/break_free/break_free_av1_854.VUEDJ3DP.webm";
import breakFreeH2641280 from "../videos/hero/break_free/break_free_h264_1280.KARTASAH.mp4";
import breakFreeH2641920 from "../videos/hero/break_free/break_free_h264_1920.IG6ELZDZ.mp4";
import breakFreeH2642560 from "../videos/hero/break_free/break_free_h264_2560.RUNGB3VA.mp4";
import breakFreeH2643840 from "../videos/hero/break_free/break_free_h264_3840.MG5X5NJZ.mp4";
import breakFreeH264426 from "../videos/hero/break_free/break_free_h264_426.RI7JWYYA.mp4";
import breakFreeH264640 from "../videos/hero/break_free/break_free_h264_640.EOYTCEFU.mp4";
import breakFreeH264854 from "../videos/hero/break_free/break_free_h264_854.YOJRKDM6.mp4";
import breakFreeVp91280 from "../videos/hero/break_free/break_free_vp9_1280.XO7Q6WAK.webm";
import breakFreeVp91920 from "../videos/hero/break_free/break_free_vp9_1920.YRJW4KBB.webm";
import breakFreeVp92560 from "../videos/hero/break_free/break_free_vp9_2560.RIK2IL3V.webm";
import breakFreeVp93840 from "../videos/hero/break_free/break_free_vp9_3840.7FNPDD33.webm";
import breakFreeVp9426 from "../videos/hero/break_free/break_free_vp9_426.DWSTPWJ6.webm";
import breakFreeVp9640 from "../videos/hero/break_free/break_free_vp9_640.VJFDHXNY.webm";
import breakFreeVp9854 from "../videos/hero/break_free/break_free_vp9_854.KEEGXMBB.webm";
...
import tokyoShuffleAv11280 from "../videos/hero/tokyo_shuffle/tokyo_shuffle_av1_1280.HOVH2ZR4.webm";
import tokyoShuffleAv11920 from "../videos/hero/tokyo_shuffle/tokyo_shuffle_av1_1920.S6SW5VQE.webm";
import tokyoShuffleAv12560 from "../videos/hero/tokyo_shuffle/tokyo_shuffle_av1_2560.SZHPXVXL.webm";
import tokyoShuffleAv13840 from "../videos/hero/tokyo_shuffle/tokyo_shuffle_av1_3840.HZGTVYOV.webm";
import tokyoShuffleAv1426 from "../videos/hero/tokyo_shuffle/tokyo_shuffle_av1_426.GAECFWY2.webm";
import tokyoShuffleAv1640 from "../videos/hero/tokyo_shuffle/tokyo_shuffle_av1_640.ORB4EHAI.webm";
import tokyoShuffleAv1854 from "../videos/hero/tokyo_shuffle/tokyo_shuffle_av1_854.WQWUD7KT.webm";
import tokyoShuffleH2641280 from "../videos/hero/tokyo_shuffle/tokyo_shuffle_h264_1280.UEW2GZ5G.mp4";
import tokyoShuffleH2641920 from "../videos/hero/tokyo_shuffle/tokyo_shuffle_h264_1920.NUVJGLAE.mp4";
import tokyoShuffleH2642560 from "../videos/hero/tokyo_shuffle/tokyo_shuffle_h264_2560.QCYDSUKP.mp4";
import tokyoShuffleH2643840 from "../videos/hero/tokyo_shuffle/tokyo_shuffle_h264_3840.O6N3I74H.mp4";
import tokyoShuffleH264426 from "../videos/hero/tokyo_shuffle/tokyo_shuffle_h264_426.FHTJAQ7F.mp4";
import tokyoShuffleH264640 from "../videos/hero/tokyo_shuffle/tokyo_shuffle_h264_640.XJR44NAQ.mp4";
import tokyoShuffleH264854 from "../videos/hero/tokyo_shuffle/tokyo_shuffle_h264_854.OGGAC5Y3.mp4";
import tokyoShuffleVp91280 from "../videos/hero/tokyo_shuffle/tokyo_shuffle_vp9_1280.DGEJFLP3.webm";
import tokyoShuffleVp91920 from "../videos/hero/tokyo_shuffle/tokyo_shuffle_vp9_1920.HOBXIKVP.webm";
import tokyoShuffleVp92560 from "../videos/hero/tokyo_shuffle/tokyo_shuffle_vp9_2560.ZJ2Y4UKP.webm";
import tokyoShuffleVp93840 from "../videos/hero/tokyo_shuffle/tokyo_shuffle_vp9_3840.2AQHLS5D.webm";
import tokyoShuffleVp9426 from "../videos/hero/tokyo_shuffle/tokyo_shuffle_vp9_426.VL5II3IX.webm";
import tokyoShuffleVp9640 from "../videos/hero/tokyo_shuffle/tokyo_shuffle_vp9_640.Z6MD7AMQ.webm";
import tokyoShuffleVp9854 from "../videos/hero/tokyo_shuffle/tokyo_shuffle_vp9_854.VFGFZLZS.webm";
...
var TimelineManager = class {
  constructor() {
    ...
    this.wrapper = (index) => {
      ...
      return (_a2 = this.timelines[nextIndex]) != null ? _a2 : this.timelines[0];
    };
  }
  ...
  onInit() {
    ...
    this.currentTimeline = this.currentTimelineIndex === -1 ? (_b = this.timelines[0]) != null ? _b : null : (_c = this.wrapper(this.currentTimelineIndex)) != null ? _c : null;
    this.paused = false;
  ...
  restart() {
    ...
    this.currentTimeline = (_a2 = this.timelines[0]) != null ? _a2 : null;
    this.paused = false;
    return this.play();
  }
  play() {
  ...
  pause() {
    var _a2;
    if ((_a2 = this.currentTimeline) == null ? void 0 : _a2.isActive()) {
  ...
  isActive() {
    var _a2;
    return (_a2 = this.currentTimeline) == null ? void 0 : _a2.isActive();
  }
  video() {
...
function srcToAttributes(src) {
  ...
  const widthPart = nameParts.length > 0 ? nameParts[nameParts.length - 1] : void 0;
  if (!widthPart) {
  ...
  const codec = nameParts.length > 1 ? nameParts[nameParts.length - 2] : void 0;
  if (!codec) {
...
function toggleActiveClass(el, classBase, makeActive) {
  ...
  Array.from(el.classList).filter((c) => makeActive ? c.endsWith("inactive") : c.endsWith("active")).filter((c) => c == null ? void 0 : c.length).forEach((c) => el.classList.remove(c));
}

// src/assets/javascripts/hero/video/videoElement.ts
var TRAILING_COMMA_REGEX = /,$/;
var VideoElement = class {
  constructor(heroVideo, properties) {
    ...
    this.video = this.createEl("video", [
      "hero__video",
      "hero__video--active"
    ]);
    this.picture = this.createEl("picture", [
      "hero__poster",
      "hero__poster--active"
    ]);
    this.backupPicture = this.createEl("picture", [
      "hero__backup"
    ]);
    this.properties = {};
    this.message = "";
    ...
    this.backupPoster = this.isSamePoster ? heroVideo.poster : ((_a2 = rawHeroVideos.find((video) => video.baseName === BACKUP_PICTURE)) == null ? void 0 : _a2.poster) || heroVideo.poster;
    ...
    for (const prop of Object.keys(this.properties)) {
      ...
      try {
        ...
        const value = (_b = this.properties[key]) != null ? _b : "";
  ...
  constructVideoElement() {
    ...
    for (const prop of Object.keys(this.properties)) {
      const key = typeof prop === "string" ? prop : "".concat(prop);
      try {
        video.setAttribute(prop, (_a2 = this.properties[key]) != null ? _a2 : "");
      } catch (e) {
        logger.error("Error setting property ".concat(key, " on video element: ").concat(e));
      }
    }
...
var VideoManager = class _VideoManager {
  constructor() {
    this.store = HeroStore.getInstance();
    this.parentContainer = document.querySelector(".hero.first .hero__container") || (() => {
      const div = document.createElement("div");
      ...
      const parent = document.querySelector(".hero.first .hero__bg") || (() => {
        ...
        requestAnimationFrame(() => {
          var _a2, _b;
          (_b = (_a2 = document.getElementsByTagName("section")[0]) == null ? void 0 : _a2.getElementsByTagName("div")[2]) == null ? void 0 : _b.append(div2);
        });
        return div2;
      })();
      parent.append(div);
    ...
    this.ctaContainer = document.querySelector(".cta__container") || (() => {
      const div = document.createElement("div");
    ...
    this.logo = (() => {
      const logoContainer = document.querySelector(".hero.first .logo__container") || (() => {
        const div = document.createElement("div");
    ...
    img == null ? void 0 : img.setAttribute("loading", "eager");
  ...
  initSubscriptions() {
    ...
    const stallHandler$ = defer(() => combineLatest([
      ...
      return canplaythrough$.pipe(filter(() => this.canPlay === true && this.element.readyState === 4 && !this.isPlaying()), exhaustMap(() => {
    ...
    const varSet$ = defer(() => fromEvent(this.element, "loadedmetadata").pipe(filter((ev) => ev && ev.target instanceof HTMLMediaElement && elementInDom(ev.target)), map(({ target }) => target), take(1), tap((target) => {
      ...
      logger.debug("Setting video dimensions from loadedmetadata event, ".concat(this.videoDuration()));
    ...
    const errorHandler$ = defer(() => of(gsapWithCSS.utils.toArray(this.element.children))).pipe(map((elementChildren) => elementChildren), switchMap((elementChildren) => {
      return merge([
    ...
    const atHome$ = state$.pipe(map(({ atHome }) => atHome), filter((atHome) => !!atHome), exhaustMap(() => {
      if (!this.initialized) {
  ...
  initVideo() {
    ...
    const promises = [];
    promises.push(new Promise(() => this.loadVideo()).then(() => {
  ...
  static getInstance() {
    if (!_VideoManager.instance) {
      _VideoManager.instance = new _VideoManager();
    }
    return _VideoManager.instance;
  }
  okToPlay() {
    return this.canPlay && !this.onFallback && !this.isPlaying() && !this.otherTimelineActive();
  }
  loadVideo() {
    this.subscriptions.add(document$2.subscribe(() => {
      this.subscriptions.add(from(new Promise(() => this.element.load())).pipe(filter(() => this.element.readyState === 4 && this.okToPlay()), catchError((err) => {
        this.handleMediaError(err);
        return EMPTY;
      })).subscribe(() => {
  ...
  handleCanPlay() {
    ...
    if (!(elementInDom(this.element) && elementInDom(this.poster)) || this.element.readyState === 0) {
      this.initVideo();
  ...
  alreadyTransitioned(revert = false) {
    return revert && this.poster.classList.contains("hero__poster--active") || !revert && this.element.classList.contains("hero__video--active");
  }
  ...
  mediaTimeline() {
    ...
    const existingOnUpdate = config6 == null ? void 0 : config6.onUpdate;
    const tl = gsapWithCSS.timeline([
      "videoTimeline",
      {
        ...config6,
        ...
        duration: 1,
        onComplete: () => {
          existingOnComplete == null ? void 0 : existingOnComplete();
        ...
        onStart: () => {
          existingOnStart == null ? void 0 : existingOnStart();
          updateDuration();
          this.transitionToVideo();
        ...
        onUpdate: () => {
          ...
          existingOnUpdate == null ? void 0 : existingOnUpdate();
        },
        paused: true
      }
    ]);
    ...
    tl.pause = (atTime, suppressEvents) => {
      tl.media.pause();
  ...
  beginPlay() {
    ...
    if (this.timelineManager.timelines.length && !this.timelineManager.isActive()) {
      ...
      } else if (!this.isPlaying() && this.timelineManager.isActive() && !this.otherTimelineActive()) {
        this.element.play();
  ...
  setVideoDimensionVars() {
    if (!this.onFallback && this.canPlay && this.element.readyState >= 3) {
      ...
      if (height && width) {
        ...
        logger.debug("Set video dimensions: ".concat(width, "x").concat(height, ", ").concat(absoluteLeft));
      }
    }
  }
  tryNewSource() {
  ...
  initiateFallback() {
    ...
    this.loadBackup();
    const newTl = gsapWithCSS.timeline({ defaults: { repeat: 0 }, paused: false });
    ...
    }).set(this.container, show()).set(gsapWithCSS.utils.toArray("*", this.ctaContainer), show()).to(this.backupPicture, { ...show(), duration: 1 });
  ...
  setupInteractions() {
    const createEventStream = (elements, eventName) => {
      const streams = elements.flatMap(({ label, iconAnchor }) => [
        fromEvent(label, eventName).pipe(map(() => ({ event: eventName, id: label.getAttribute("for") || "" }))),
        fromEvent(iconAnchor, eventName).pipe(map(() => ({ event: eventName, id: iconAnchor.id.replace("icon-", "") })))
      ]);
    ...
    const iconClicks = this.tabs.map(({ iconAnchor, input, labelAnchor }) => merge(fromEvent(labelAnchor, "click"), fromEvent(iconAnchor, "click")).pipe(tap((event) => {
    ...
    const childClicks = this.childTabs.map(({ input, labelAnchor }) => fromEvent(labelAnchor, "click").pipe(tap((event) => {
      preventDefault(event);
      if (!input.checked) {
        input.checked = true;
...
// src/assets/javascripts/licenses/init.ts
...
var license$ = navigationEvents$.pipe(filter((url2) => !!isLicense(url2)), switchMap(() => initLicenseFeature()));
var fixSvg$ = onDom$(of(fixSvgDimensions()));
var windowEvents$ = onDom$(of(windowEvents()));
var pageConfigs = [
  {
    location: "home",
    matcher: isHome,
...
gsap/ScrollTrigger.js:
  ...
  *)
*/
//# sourceMappingURL=index.YEEIZZGJ.js.map
