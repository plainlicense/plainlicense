                ...
                var ClipboardActionCopy = function ClipboardActionCopy2(target) {
                  ...
                  if (typeof target === 'string') {
                    ...
                    !['text', 'search', 'url', 'tel', 'password'].includes(
                      target === null || target === void 0 ? void 0 : target.type,
                    )
                  ) {
    ...
    (function () {
      ...
      Tablesort2.prototype = {
        ...
        sortTable: function (header2, update) {
          var that = this,
            columnKey = header2.getAttribute('data-sort-column-key'),
            column = header2.cellIndex,
            sortFunction = caseInsensitiveSort,
            item = '',
            items = [],
            i = that.thead ? 0 : 1,
            sortMethod = header2.getAttribute('data-sort-method'),
            sortRevers = header2.hasAttribute('data-sort-reverse'),
            sortOrder = header2.getAttribute('aria-sort');
          ...
          for (i = 0; i < that.table.tBodies.length; i++) {
            ...
            if (
              (sortOrder === 'descending' && !sortRevers) ||
              (sortOrder === 'ascending' && sortRevers)
            ) {
...
var _setDuration = function _setDuration2(animation, duration, skipUncache, leavePlayhead) {
  var repeat2 = animation._repeat,
    dur = _roundPrecise(duration) || 0,
    totalProgress = animation._tTime / animation._tDur;
  totalProgress && !leavePlayhead && (animation._time *= dur / animation._dur);
  animation._dur = dur;
  animation._tDur = !repeat2
    ? dur
    : repeat2 < 0
      ? 1e10
      : _roundPrecise(dur * (repeat2 + 1) + animation._rDelay * repeat2);
  totalProgress > 0 &&
    !leavePlayhead &&
...
var _parsePosition = function _parsePosition2(animation, position, percentAnimation) {
  ...
  if (_isString(position) && (isNaN(position) || position in labels)) {
    ...
    return i > 1
      ? _parsePosition2(animation, position.substr(0, i - 1), percentAnimation) + offset
...
var distribute = function distribute2(v) {
  ...
  return function (i, target, a) {
    ...
    if (!distances) {
      ...
      originY =
        wrapAt === _bigNum ? 0 : ratios ? (l * ratioY) / wrapAt - 0.5 : (from2 / wrapAt) | 0;
      max = 0;
      min = _bigNum;
      for (j = 0; j < l; j++) {
        x = (j % wrapAt) - originX;
...
var _replaceRandom = function _replaceRandom2(value) {
  ...
  while (~(i = value.indexOf('random(', prev))) {
    ...
    s +=
      ...
      random(isArray4 ? nums : +nums[0], isArray4 ? 0 : +nums[1], +nums[2] || 1e-5);
    prev = end + 1;
  }
...
var _createPlugin = function _createPlugin2(config6) {
  ...
  if (_windowExists() || config6.headless) {
    var name = config6.name,
      isFunc = _isFunction(config6),
      Plugin =
        name && !isFunc && config6.init
          ? function () {
              this._props = [];
            }
          : config6,
...
var _renderComplexString = function _renderComplexString2(ratio, data) {
  ...
  if (!ratio && data.b) {
    ...
    while (pt) {
      s =
        pt.p +
        (pt.m ? pt.m(pt.s + pt.c * ratio) : Math.round((pt.s + pt.c * ratio) * 1e4) / 1e4) +
        s;
      pt = pt._next;
    }
    s += data.c;
      ...
      (function () {
        ...
        tweens
          .map(function (t2) {
            return {
              g:
                t2._dur || t2._delay || (t2._sat && !t2._sat.vars.immediateRender)
                  ? t2.globalTime(0)
                  : -Infinity,
              t: t2,
            };
...
var MatchMedia = /* @__PURE__ */ (function () {
  ...
  _proto6.add = function add(conditions, func, scope) {
    ...
    var context4 = new Context(0, scope || this.scope),
      cond = (context4.conditions = {}),
      mq,
      p,
      active;
...
var _gsap = {
  ...
  quickTo: function quickTo(target, property, vars) {
    ...
    var tween = gsap.to(
        ...
        _setDefaults(
          ((_setDefaults22 = {}),
          (_setDefaults22[property] = '+=0.1'),
          (_setDefaults22.paused = true),
          (_setDefaults22.stagger = 0),
          _setDefaults22),
          vars || {},
        ),
      ),
...
var _renderCSSPropWithBeginning = function _renderCSSPropWithBeginning2(ratio, data) {
  return data.set(
    data.t,
    data.p,
    ratio ? Math.round((data.s + data.c * ratio) * 1e4) / 1e4 + data.u : data.b,
    data,
  );
};
...
var _createElement = function _createElement2(type, ns) {
  var e = _doc2.createElementNS
    ? _doc2.createElementNS((ns || 'http://www.w3.org/1999/xhtml').replace(/^https/, 'http'), type)
...
var _getComputedProperty = function _getComputedProperty2(target, property, skipPrefixFallback) {
  ...
  return (
    ...
    (!skipPrefixFallback &&
      _getComputedProperty2(target, _checkPropPrefix(property) || property, 1)) ||
    ''
  );
};
...
var VIDEO_MANAGER_ELEMENTS =
  '.hero__video, .hero__poster, .hero__poster--image, .hero__backup, noscript, style, script';
var MAX_WIDTHS = {
  426: '426',
  640: '640',
  854: '854',
...
var isDev = (url2) => {
  return (
    (url2.hostname === 'localhost' && url2.port === '8000') ||
    (url2.hostname === '127.0.0.1' && url2.port === '8000')
  );
};
var isOnSite = (url2) => {
...
var isMainSiteLicensePage = (url2) => {
  return (
    (url2.pathname.endsWith('index.html') && url2.pathname.split('/').length === 5) ||
    (url2.pathname.endsWith('/') && url2.pathname.split('/').length === 4)
  );
};
...
var isHelpingIndex = (url2) => {
  return (
    url2.pathname.includes('helping') &&
    ((url2.pathname.split('/').length === 3 && url2.pathname.endsWith('index.html')) ||
      (url2.pathname.split('/').length === 2 && url2.pathname.endsWith('/')))
  );
};
...
function elementInDom(el, container) {
...
var logger = {
  debug: (message, ...args) => {
    if (isDevelopment) {
      console.debug('%c[DEBUG] '.concat(message), 'color: aqua', ...args);
    }
  },
  error: (message, ...args) => {
    if (isDevelopment) {
      console.error('%c[ERROR] '.concat(message), 'color: magenta', ...args);
    }
  },
  warn: (message, ...args) => {
    if (isDevelopment) {
      console.warn('%c[WARNING] '.concat(message), 'color: orange', ...args);
    }
  },
  info: (message, ...args) => {
    if (isDevelopment) {
      console.info('%c[INFO] '.concat(message), 'color: white', ...args);
    }
  },
  table: (data) => {
    if (isDevelopment) {
...
var createScript = (src, async2 = true, defer2 = true, ignoreDnt = false) => {
  const alreadyLoaded = document.querySelector('script[src="'.concat(src, '"]'));
  if (alreadyLoaded) {
    return;
  }
...
function parsePath(path) {
  ...
  try {
    ...
    urlObj = {
      ...
      search: url2.search.length
        ? Object.fromEntries(new URLSearchParams(url2.search).entries()) || ''
        : url2.search || '',
...
function logObject(obj, label = '') {
  if (!obj) {
    logger.error('No object to log for '.concat(label));
    return;
  }
  if (typeof obj === 'string' || typeof obj === 'number' || typeof obj === 'boolean') {
    logger.debug(''.concat(label, ': '), obj);
    return;
  }
  if (Array.isArray(obj)) {
    obj.forEach((item, i) => {
      logger.debug(''.concat(label, '[').concat(i, ']: '), item);
    });
    return;
  } else if (typeof obj === 'object') {
    Object.keys(obj).forEach((key) => {
      logger.debug(''.concat(label, '.').concat(key, ': '), obj[key]);
    });
  } else {
    logger.debug(''.concat(label, ': '), obj);
  }
...
function setNavId() {
  const nav2 = document.querySelector('nav.md-tabs');
  if (!nav2) {
    logger.error('No nav element found');
    return;
  }
  nav2.setAttribute('id', 'hero-tabs');
  logger.debug("Set nav id to 'hero-tabs'");
}
function removeHiddenAttr(element) {
  ...
  Subscription2.prototype.add = function (teardown) {
    ...
    if (teardown && teardown !== this) {
      if (this.closed) {
        ...
        (this._finalizers = (_a2 = this._finalizers) !== null && _a2 !== void 0 ? _a2 : []).push(
          teardown,
        );
      }
    }
...
function getPromiseCtor(promiseCtor) {
  var _a2;
  return (_a2 = promiseCtor !== null && promiseCtor !== void 0 ? promiseCtor : config3.Promise) !==
    null && _a2 !== void 0
    ? _a2
    : Promise;
}
...
var animationFrameProvider = {
  ...
  requestAnimationFrame: function () {
    ...
    var delegate = animationFrameProvider.delegate;
    return (
      (delegate === null || delegate === void 0 ? void 0 : delegate.requestAnimationFrame) ||
      requestAnimationFrame
  ...
  cancelAnimationFrame: function () {
    ...
    var delegate = animationFrameProvider.delegate;
    return (
      (delegate === null || delegate === void 0 ? void 0 : delegate.cancelAnimationFrame) ||
...
var Subject = (function (_super) {
  ...
  Subject8.prototype._innerSubscribe = function (subscriber) {
    var _this = this;
    var _a2 = this,
      hasError = _a2.hasError,
      isStopped = _a2.isStopped,
      observers = _a2.observers;
...
var AnonymousSubject = (function (_super) {
  ...
  AnonymousSubject2.prototype.next = function (value) {
    var _a2, _b;
    (_b = (_a2 = this.destination) === null || _a2 === void 0 ? void 0 : _a2.next) === null ||
    _b === void 0
      ? void 0
      : _b.call(_a2, value);
  };
  AnonymousSubject2.prototype.error = function (err) {
    var _a2, _b;
    (_b = (_a2 = this.destination) === null || _a2 === void 0 ? void 0 : _a2.error) === null ||
    _b === void 0
      ? void 0
      : _b.call(_a2, err);
  };
  AnonymousSubject2.prototype.complete = function () {
    var _a2, _b;
    (_b = (_a2 = this.destination) === null || _a2 === void 0 ? void 0 : _a2.complete) === null ||
    _b === void 0
      ? void 0
      : _b.call(_a2);
  };
  AnonymousSubject2.prototype._subscribe = function (subscriber) {
    var _a2, _b;
    return (_b =
      (_a2 = this.source) === null || _a2 === void 0 ? void 0 : _a2.subscribe(subscriber)) !==
      null && _b !== void 0
      ? _b
      : EMPTY_SUBSCRIPTION;
  };
...
var ReplaySubject = (function (_super) {
  ...
  ReplaySubject2.prototype.next = function (value) {
    var _a2 = this,
      isStopped = _a2.isStopped,
      _buffer = _a2._buffer,
      _infiniteTimeWindow = _a2._infiniteTimeWindow,
      _timestampProvider = _a2._timestampProvider,
      _windowTime = _a2._windowTime;
    if (!isStopped) {
      _buffer.push(value);
...
var intervalProvider = {
  ...
  clearInterval: function (handle2) {
    var delegate = intervalProvider.delegate;
    return (
      (delegate === null || delegate === void 0 ? void 0 : delegate.clearInterval) || clearInterval
    )(handle2);
  },
  delegate: void 0,
};
...
var AnimationFrameAction = (function (_super) {
  ...
  AnimationFrameAction2.prototype.recycleAsyncId = function (scheduler, id, delay2) {
    ...
    if (
      ...
      ((_a2 = actions[actions.length - 1]) === null || _a2 === void 0 ? void 0 : _a2.id) !== id
    ) {
...
function scheduleIterable(input, scheduler) {
  return new Observable(function (subscriber) {
    ...
    return function () {
      return (
        isFunction(iterator2 === null || iterator2 === void 0 ? void 0 : iterator2.return) &&
        iterator2.return()
      );
    };
  });
...
function combineLatest() {
  ...
  var _a2 = argsArgArrayOrObject(args),
    observables = _a2.args,
    keys = _a2.keys;
            ...
            createOperatorSubscriber(
              ...
              function (value) {
                ...
                if (
                  ...
                  subscriber.next(
                    resultSelector
                      ? resultSelector.apply(void 0, __spreadArray([], __read(result)))
                      : result,
                  );
                  if (
...
function merge2() {
  ...
  var concurrent = popNumber(args, Infinity);
  return operate(function (source, subscriber) {
    mergeAll(concurrent)(from(__spreadArray([source], __read(args)), scheduler)).subscribe(
      subscriber,
    );
  });
}
...
function repeat(countOrConfig) {
  ...
  if (countOrConfig != null) {
    if (typeof countOrConfig === 'object') {
      (_a2 = countOrConfig.count),
        (count = _a2 === void 0 ? Infinity : _a2),
...
function shareReplay(configOrBufferSize, windowTime, scheduler) {
  ...
  if (configOrBufferSize && typeof configOrBufferSize === 'object') {
    (_a2 = configOrBufferSize.bufferSize),
      (bufferSize = _a2 === void 0 ? Infinity : _a2),
    ...
    bufferSize =
      configOrBufferSize !== null && configOrBufferSize !== void 0 ? configOrBufferSize : Infinity;
  }
  return share({
    connector: function () {
    ...
    ? operate(function (source, subscriber) {
        ...
        source.subscribe(
          createOperatorSubscriber(
            ...
            function (value) {
              var _a3;
              (_a3 = tapObserver.next) === null || _a3 === void 0
                ? void 0
                : _a3.call(tapObserver, value);
              subscriber.next(value);
            },
            function () {
              var _a3;
              isUnsub = false;
              (_a3 = tapObserver.complete) === null || _a3 === void 0
                ? void 0
                : _a3.call(tapObserver);
              subscriber.complete();
            },
            function (err) {
              var _a3;
              isUnsub = false;
              (_a3 = tapObserver.error) === null || _a3 === void 0
                ? void 0
                : _a3.call(tapObserver, err);
              subscriber.error(err);
            },
            function () {
              var _a3, _b;
              if (isUnsub) {
                (_a3 = tapObserver.unsubscribe) === null || _a3 === void 0
                  ? void 0
                  : _a3.call(tapObserver);
              }
...
function throttle(durationSelector, config6) {
  return operate(function (source, subscriber) {
    var _a2 = config6 !== null && config6 !== void 0 ? config6 : {},
      _b = _a2.leading,
...
function watchDocument() {
  ...
  fromEvent(document, 'DOMContentLoaded', { once: true }).subscribe(() =>
    document$5.next(document),
  );
  return document$5;
}
...
function getElement(selector3, node = document) {
  ...
  if (typeof el === 'undefined')
    throw new ReferenceError('Missing element: expected "'.concat(selector3, '" to be present'));
  return el;
}
...
function getActiveElement() {
  ...
  return (_d =
    (_c =
      (_b = (_a2 = document.activeElement) == null ? void 0 : _a2.shadowRoot) == null
        ? void 0
        : _b.activeElement) != null
      ? _c
...
function h(tag, attributes, ...children) {
  ...
  if (attributes)
    for (const attr of Object.keys(attributes)) {
      if (typeof attributes[attr] === 'undefined') continue;
      if (typeof attributes[attr] !== 'boolean') el.setAttribute(attr, attributes[attr]);
...
function round(value) {
  if (value > 999) {
    ...
    return ''.concat(((value + 1e-6) / 1e3).toFixed(digits), 'k');
  } else {
  ...
  return defer(() => {
    ...
    return merge(
      ...
      fromEvent(script2, 'error').pipe(
        switchMap(() => throwError(() => new ReferenceError('Invalid script: '.concat(src)))),
      ),
    ).pipe(
      map(() => void 0),
...
var observer$2 = defer(() =>
  ...
  switchMap((observer) => merge(NEVER, of(observer)).pipe(finalize(() => observer.disconnect()))),
  shareReplay(1),
);
...
function getElementContainers(el) {
  ...
  if (containers.length === 0) containers.push(document.documentElement);
  return containers;
}
...
function watchElementOffset(el) {
  return merge(fromEvent(window, 'load'), fromEvent(window, 'resize')).pipe(
...
var observer$3 = defer(() =>
  of(
    new IntersectionObserver(
      (entries) => {
        for (const entry of entries) entry$2.next(entry);
      },
      {
        threshold: 0,
      },
...
function isSusceptibleToKeyboard(el, type) {
  switch (el.constructor) {
    ...
    case HTMLInputElement:
      if (el.type === 'radio') return /^Arrow/.test(type);
      else return true;
  ...
  const keyboard$2 = fromEvent(window, 'keydown').pipe(
    ...
    filter(({ mode, type }) => {
      if (mode === 'global') {
        ...
        if (typeof active !== 'undefined') return !isSusceptibleToKeyboard(active, type);
      }
      return true;
    }),
    share(),
  );
  return watchComposition().pipe(switchMap((active) => (!active ? keyboard$2 : EMPTY)));
}
...
function watchLocationHash(location$3) {
  return merge(fromEvent(window, 'hashchange'), location$3).pipe(
    map(getLocationHash),
...
function at(query$, factory) {
  return query$.pipe(switchMap((active) => (active ? factory() : EMPTY)));
}
...
function watchViewport() {
  return combineLatest([watchViewportOffset(), watchViewportSize()]).pipe(
...
function watchViewportAt(el, { viewport$: viewport$5, header$: header$3 }) {
  const size$ = viewport$5.pipe(distinctUntilKeyChanged('size'));
  const offset$ = combineLatest([size$, header$3]).pipe(map(() => getElementOffset(el)));
...
function watchWorker(url2, worker = new Worker(url2)) {
  ...
  return worker$.pipe(ignoreElements(), mergeWith(recv$.pipe(takeUntil(done$))), share());
}
...
config4.base = ''.concat(new URL(config4.base, getLocation()));
...
function translation(key, value) {
  return typeof value !== 'undefined'
    ? config4.translations[key].replace('#', value.toString())
    : config4.translations[key];
}
...
function getComponentElement(type, node = document) {
  return getElement('[data-md-component='.concat(type, ']'), node);
}
function getComponentElements(type, node = document) {
  return getElements('[data-md-component='.concat(type, ']'), node);
}
...
function mountAnnounce(el) {
  ...
  if (!el.hidden) {
    ...
    if (__md_hash(content.innerHTML) === __md_get('__announce')) el.hidden = true;
  }
  return defer(() => {
...
function watchConsent(el, { target$: target$2 }) {
  return target$2.pipe(map((target) => ({ hidden: target !== el })));
}
...
function renderTooltip(id, style) {
  if (style === 'inline') {
    return /* @__PURE__ */ h(
      'div',
      { class: 'md-tooltip md-tooltip--inline', id, role: 'tooltip' },
      /* @__PURE__ */ h('div', { class: 'md-tooltip__inner md-typeset' }),
    );
  } else {
    return /* @__PURE__ */ h(
      'div',
      { class: 'md-tooltip', id, role: 'tooltip' },
      /* @__PURE__ */ h('div', { class: 'md-tooltip__inner md-typeset' }),
    );
  }
}
function renderInlineTooltip2(...children) {
  return /* @__PURE__ */ h(
    ...
    /* @__PURE__ */ h('div', { class: 'md-tooltip2__inner md-typeset' }, children),
  );
}
...
function renderAnnotation(id, prefix) {
  prefix = prefix ? ''.concat(prefix, '_annotation_').concat(id) : void 0;
  if (prefix) {
    const anchor = prefix ? '#'.concat(prefix) : void 0;
    return /* @__PURE__ */ h(
      'aside',
      ...
      /* @__PURE__ */ h(
        'a',
        { href: anchor, class: 'md-annotation__index', tabIndex: -1 },
        /* @__PURE__ */ h('span', { 'data-md-annotation-id': id }),
      ),
    );
  } else {
    return /* @__PURE__ */ h(
...
function renderClipboardButton(id) {
  return /* @__PURE__ */ h('button', {
    ...
    'data-clipboard-target': '#'.concat(id, ' > code'),
  });
}
...
function renderSearchDocument(document2, flag) {
  ...
  const missing = Object.keys(document2.terms)
    .filter((key) => !document2.terms[key])
    .reduce(
      (list, key) => [
        ...list,
  ...
  if (feature('search.highlight'))
    url2.searchParams.set(
      'h',
      Object.entries(document2.terms)
        .filter(([, match]) => match)
        .reduce((highlight2, [value]) => ''.concat(highlight2, ' ').concat(value).trim(), ''),
    );
  const { tags } = configuration();
  return /* @__PURE__ */ h(
    'a',
    { href: ''.concat(url2), class: 'md-search-result__link', tabIndex: -1 },
    /* @__PURE__ */ h(
      'article',
      {
      ...
      document2.tags &&
        /* @__PURE__ */ h(
          'nav',
          { class: 'md-tags' },
          document2.tags.map((tag) => {
            const type = tags
              ? tag in tags
                ? 'md-tag-icon md-tag--'.concat(tags[tag])
                : 'md-tag-icon'
              : '';
            return /* @__PURE__ */ h('span', { class: 'md-tag '.concat(type) }, tag);
          }),
        ),
      teaser > 0 &&
        missing.length > 0 &&
        /* @__PURE__ */ h(
          ...
          translation('search.result.term.missing'),
          ': ',
          ...missing,
        ),
    ),
...
function renderSearchResultItem(result) {
  ...
  const parent = docs.findIndex((doc) => {
    const l = ''.concat(new URL(doc.location, config6.base));
    return !l.includes('#');
  });
    ...
    ...(more.length
      ? [
          /* @__PURE__ */ h(
            ...
            ...more.map((section) => renderSearchDocument(section, 1 /* TEASER */)),
          ),
        ]
      : []),
  ];
...
function renderSourceFacts(facts) {
  return /* @__PURE__ */ h(
    ...
    Object.entries(facts).map(([key, value]) =>
      /* @__PURE__ */ h(
        'li',
        { class: 'md-source__fact md-source__fact--'.concat(key) },
...
function renderTabbedControl(type) {
  const classes = 'tabbed-control tabbed-control--'.concat(type);
  return /* @__PURE__ */ h(
    'div',
    { class: classes, hidden: true },
    /* @__PURE__ */ h('button', { class: 'tabbed-button', tabIndex: -1, 'aria-hidden': 'true' }),
  );
}
...
function renderTable(table) {
  return /* @__PURE__ */ h(
    ...
    /* @__PURE__ */ h('div', { class: 'md-typeset__table' }, table),
  );
}
...
function renderVersion(version) {
  var _a2;
  const config6 = configuration();
  const url2 = new URL('../'.concat(version.version, '/'), config6.base);
  return /* @__PURE__ */ h(
    'li',
    { class: 'md-version__item' },
    /* @__PURE__ */ h(
      'a',
      { href: ''.concat(url2), class: 'md-version__link' },
      version.title,
      ((_a2 = config6.version) == null ? void 0 : _a2.alias) &&
        version.aliases.length > 0 &&
        /* @__PURE__ */ h('span', { class: 'md-version__alias' }, version.aliases[0]),
    ),
  );
}
function renderVersionSelector(versions, active) {
  ...
  versions = versions.filter((version) => {
    var _a3;
    return !((_a3 = version.properties) == null ? void 0 : _a3.hidden);
  });
  return /* @__PURE__ */ h(
    'div',
    { class: 'md-version' },
    /* @__PURE__ */ h(
      ...
      ((_a2 = config6.version) == null ? void 0 : _a2.alias) &&
        ...
        /* @__PURE__ */ h('span', { class: 'md-version__alias' }, active.aliases[0]),
    ),
...
function watchTooltip2(el) {
  const active$ = combineLatest([watchElementFocus(el), watchElementHover(el)]).pipe(
...
function mountTooltip2(el, dependencies) {
  ...
  const id = '__tooltip2_'.concat(sequence++);
  return defer(() => {
    const push$ = new Subject();
    ...
    combineLatest([node$, push$, origin$]).subscribe(([node, { offset }, origin2]) => {
      node.style.setProperty('--md-tooltip-host-x', ''.concat(offset.x, 'px'));
      node.style.setProperty('--md-tooltip-host-y', ''.concat(offset.y, 'px'));
      node.style.setProperty('--md-tooltip-x', ''.concat(origin2.x, 'px'));
      node.style.setProperty('--md-tooltip-y', ''.concat(origin2.y, 'px'));
      node.classList.toggle('md-tooltip2--top', origin2.y < 0);
      node.classList.toggle('md-tooltip2--bottom', origin2.y >= 0);
    });
    show$
      .pipe(
        filter((active) => active),
      ...
      .subscribe((node) => {
        ...
        node.style.setProperty('--md-tooltip-width', ''.concat(size.width, 'px'));
        node.style.setProperty('--md-tooltip-tail', ''.concat(0, 'px'));
      });
    show$
      .pipe(distinctUntilChanged(), observeOn(animationFrameScheduler), withLatestFrom(node$))
      .subscribe(([active, node]) => {
        node.classList.toggle('md-tooltip2--active', active);
      });
    combineLatest([show$.pipe(filter((active) => active)), node$]).subscribe(([_, node]) => {
      if (node.role === 'dialog') {
...
function watchAnnotation(el, container) {
  const offset$ = defer(() =>
    combineLatest([watchElementOffset(el), watchElementContentOffset(container)]),
  ).pipe(
    map(([{ x, y }, scroll]) => {
...
function mountAnnotation(el, container, { target$: target$2 }) {
  ...
  return defer(() => {
    ...
    push$.subscribe({
      /* Handle emission */
      next({ offset }) {
        el.style.setProperty('--md-tooltip-x', ''.concat(offset.x, 'px'));
        el.style.setProperty('--md-tooltip-y', ''.concat(offset.y, 'px'));
      },
      /* Handle complete */
      complete() {
    ...
    push$.pipe(auditTime(16, animationFrameScheduler)).subscribe(({ active }) => {
    ...
    push$
      ...
      .subscribe({
        /* Handle emission */
        next(origin2) {
          if (origin2) el.style.setProperty('--md-tooltip-0', ''.concat(-origin2, 'px'));
          else el.style.removeProperty('--md-tooltip-0');
        },
        /* Handle complete */
        complete() {
    ...
    fromEvent(index, 'mousedown')
      .pipe(takeUntil(done$), withLatestFrom(push$))
      .subscribe(([ev, { active }]) => {
        var _a2;
        if (ev.button !== 0 || ev.metaKey || ev.ctrlKey) {
          ...
          else (_a2 = getActiveElement()) == null ? void 0 : _a2.blur();
        }
      });
    target$2
      .pipe(
...
function findMarkers(container) {
  ...
  for (const el of findHosts(container)) {
    ...
    for (let node = it.nextNode(); node; node = it.nextNode()) nodes.push(node);
...
function mountAnnotationList(el, container, { target$: target$2, print$: print$2 }) {
  ...
  for (const marker of findMarkers(container)) {
    ...
    if (getOptionalElement(':scope > li:nth-child('.concat(id, ')'), el)) {
      annotations.set(id, renderAnnotation(id, prefix));
      marker.replaceWith(annotations.get(id));
    }
  }
  if (annotations.size === 0) return EMPTY;
  return defer(() => {
    const push$ = new Subject();
    const done$ = push$.pipe(ignoreElements(), endWith(true));
    const pairs = [];
    for (const [id, annotation] of annotations)
      pairs.push([
        ...
        getElement(':scope > li:nth-child('.concat(id, ')'), el),
      ]);
    print$2.pipe(takeUntil(done$)).subscribe((active) => {
      el.hidden = !active;
      el.classList.toggle('md-annotation-list', active);
      for (const [inner, child] of pairs)
        if (!active) swap(child, inner);
        else swap(inner, child);
    });
    return merge(
      ...[...annotations].map(([, annotation]) =>
        mountAnnotation(annotation, container, { target$: target$2 }),
      ),
    ).pipe(
      finalize(() => push$.complete()),
      share(),
...
function findList(el) {
  if (el.nextElementSibling) {
    ...
    else if (sibling.tagName === 'P' && !sibling.children.length) return findList(sibling);
  }
  return void 0;
}
...
function findCandidateList(el) {
  if (el.nextElementSibling) {
    const sibling = el.nextElementSibling;
    if (sibling.tagName === 'OL') return sibling;
    else if (sibling.tagName === 'P' && !sibling.children.length) return findCandidateList(sibling);
  }
  return void 0;
}
...
function mountCodeBlock(el, options) {
  ...
  const factory$ = defer(() => {
    ...
    push$.subscribe(({ scrollable }) => {
      if (scrollable && hover) el.setAttribute('tabindex', '0');
    ...
    if (import_clipboard.default.isSupported()) {
      if (el.closest('.copy') || (feature('content.code.copy') && !el.closest('.no-copy'))) {
        ...
        parent.id = '__code_'.concat(sequence2++);
        const button = renderClipboardButton(parent.id);
        parent.insertBefore(button, el);
        if (feature('content.tooltips')) content$2.push(mountInlineTooltip2(button, { viewport$ }));
      }
    }
    ...
    if (container instanceof HTMLElement) {
      ...
      if (
        ...
        (container.classList.contains('annotate') || feature('content.code.annotate'))
      ) {
    ...
    if (spans.length) el.classList.add('md-code__content');
...
function mountMermaid(el) {
  ...
  mermaid$.subscribe(async () => {
    ...
    const id = '__mermaid_'.concat(sequence3++);
  ...
  return mermaid$.pipe(map(() => ({ ref: el })));
}
...
function watchContentTabs(inputs) {
  ...
  return merge(
    ...inputs.map((input) =>
      fromEvent(input, 'change').pipe(map(() => getElement('label[for="'.concat(input.id, '"]')))),
    ),
  ).pipe(
    startWith(getElement('label[for="'.concat(initial.id, '"]'))),
    map((active) => ({ active })),
  );
}
function mountContentTabs(el, { viewport$: viewport$5, target$: target$2 }) {
  ...
  return defer(() => {
    ...
    combineLatest([push$, watchElementSize(el), watchElementVisibility(el)])
      .pipe(takeUntil(done$), auditTime(1, animationFrameScheduler))
      .subscribe({
        /* Handle emission */
        next([{ active }, size]) {
          const offset = getElementOffset(active);
          const { width } = getElementSize(active);
          el.style.setProperty('--md-indicator-x', ''.concat(offset.x, 'px'));
          el.style.setProperty('--md-indicator-width', ''.concat(width, 'px'));
    ...
    combineLatest([watchElementContentOffset(container), watchElementSize(container)])
      .pipe(takeUntil(done$))
    ...
    merge(fromEvent(prev, 'click').pipe(map(() => -1)), fromEvent(next, 'click').pipe(map(() => 1)))
      .pipe(takeUntil(done$))
      .subscribe((direction) => {
    ...
    for (const input of inputs) {
      const label = getElement('label[for="'.concat(input.id, '"]'));
      label.replaceChildren(
        h(
          'a',
          {
            href: '#'.concat(label.htmlFor),
            tabIndex: -1,
          },
          ...Array.from(label.childNodes),
        ),
      );
      fromEvent(label.firstElementChild, 'click')
        .pipe(
          takeUntil(done$),
          filter((ev) => !(ev.metaKey || ev.ctrlKey)),
          tap((ev) => {
        ...
        .subscribe(() => {
          history.replaceState({}, '', '#'.concat(label.htmlFor));
          label.click();
        });
    }
    if (feature('content.tabs.link'))
      push$.pipe(skip(1), withLatestFrom(viewport$5)).subscribe(([{ active }, { offset }]) => {
    ...
    push$.pipe(takeUntil(done$)).subscribe(() => {
      for (const media of getElements('audio, video', el)) media.pause();
    });
...
function mountContent(el, { viewport$: viewport$5, target$: target$2, print$: print$2 }) {
  return merge(
    ...getElements('.annotate:not(.highlight)', el).map((child) =>
      mountAnnotationBlock(child, { target$: target$2, print$: print$2 }),
    ),
    ...getElements('pre:not(.mermaid) > code', el).map((child) =>
      mountCodeBlock(child, { target$: target$2, print$: print$2 }),
    ),
    ...
    ...getElements('details', el).map((child) =>
      mountDetails(child, { target$: target$2, print$: print$2 }),
    ),
    ...getElements('[data-tabs]', el).map((child) =>
      mountContentTabs(child, { viewport$: viewport$5, target$: target$2 }),
    ),
    ...getElements('[title]', el)
      .filter(() => feature('content.tooltips'))
      .map((child) => mountInlineTooltip2(child, { viewport$: viewport$5 })),
  );
}
...
function watchDialog(_el, { alert$: alert$2 }) {
  return alert$2.pipe(
    switchMap((message) =>
      merge(of(true), of(false).pipe(delay(2e3))).pipe(map((active) => ({ message, active }))),
    ),
  );
}
...
function watchTooltip(el, host) {
  ...
  el.style.setProperty('--md-tooltip-width', ''.concat(width, 'px'));
  el.remove();
  const container = getElementContainer(host);
  const scroll$ =
    typeof container !== 'undefined' ? watchElementContentOffset(container) : of({ x: 0, y: 0 });
  const active$ = merge(watchElementFocus(host), watchElementHover(host)).pipe(
    distinctUntilChanged(),
  );
...
function mountTooltip(el) {
  const title = el.title;
  if (!title.length) return EMPTY;
  const id = '__tooltip_'.concat(sequence4++);
  ...
  return defer(() => {
    ...
    push$.subscribe({
      /* Handle emission */
      next({ offset }) {
        tooltip.style.setProperty('--md-tooltip-x', ''.concat(offset.x, 'px'));
        tooltip.style.setProperty('--md-tooltip-y', ''.concat(offset.y, 'px'));
      },
      /* Handle complete */
      complete() {
    ...
    push$
      ...
      .subscribe({
        /* Handle emission */
        next(origin2) {
          if (origin2) tooltip.style.setProperty('--md-tooltip-0', ''.concat(-origin2, 'px'));
          else tooltip.style.removeProperty('--md-tooltip-0');
        },
        /* Handle complete */
        complete() {
...
function mountHeader(el, { header$: header$3, main$: main$2 }) {
  return defer(() => {
    ...
    const done$ = push$.pipe(ignoreElements(), endWith(true));
    push$
      .pipe(distinctUntilKeyChanged('active'), combineLatestWith(header$3))
...
function watchMain(el, { viewport$: viewport$5, header$: header$3 }) {
  ...
  return combineLatest([adjust$, border$, viewport$5]).pipe(
    ...
    distinctUntilChanged(
      (a, b) => a.offset === b.offset && a.height === b.height && a.active === b.active,
    ),
  );
}
...
function watchPalette(inputs) {
  const current = __md_get('__palette') || {
    index: inputs.findIndex(
      (input) => matchMedia(input.getAttribute('data-md-color-media')).matches,
    ),
  };
  ...
  return defer(() => {
    ...
    push$.subscribe((palette) => {
      ...
      for (const [key, value] of Object.entries(palette.color))
        document.body.setAttribute('data-md-color-'.concat(key), value);
      for (let index = 0; index < inputs.length; index++) {
        const label = inputs[index].nextElementSibling;
        if (label instanceof HTMLElement) label.hidden = palette.index !== index;
      }
    ...
    push$
      ...
      .subscribe((color) => (meta.content = '#'.concat(color)));
...
function mountProgress(el, { progress$: progress$2 }) {
  return defer(() => {
    ...
    push$.subscribe(({ value }) => {
      el.style.setProperty('--md-progress-value', ''.concat(value));
    });
    ...
    new Observable((subscriber) => {
      new import_clipboard2.default('[data-clipboard-target], [data-clipboard-text]', {
        text: (el) =>
          ...
          extract(getElement(el.getAttribute('data-clipboard-target'))),
...
function extract2(document2, base) {
  ...
  for (const el of getElements('url', document2)) {
    ...
    sitemap.set(''.concat(links[0]), links);
...
function handle(ev, sitemap) {
  if (!(ev.target instanceof Element)) return EMPTY;
  const el = ev.target.closest('a');
  if (el === null) return EMPTY;
  if (el.target || ev.metaKey || ev.ctrlKey) return EMPTY;
  const url2 = new URL(el.href);
  url2.search = url2.hash = '';
  if (!sitemap.has(''.concat(url2))) return EMPTY;
  ev.preventDefault();
  return of(new URL(el.href));
}
function head(document2) {
  const tags = /* @__PURE__ */ new Map();
  for (const el of getElements(':scope > *', document2.head)) tags.set(el.outerHTML, el);
  return tags;
}
...
function inject(next) {
  ...
  for (const [html, el] of head(next))
    if (tags.has(html)) tags.delete(html);
    else document.head.appendChild(el);
  for (const el of tags.values()) {
    const name = el.getAttribute('name');
    if (name !== 'theme-color' && name !== 'color-scheme') el.remove();
  }
...
function setupInstantNavigation({
  ...
  const config6 = configuration();
  if (location.protocol === 'file:') return EMPTY;
  ...
  const history$ = fromEvent(window, 'popstate').pipe(map(getLocation), share());
  ...
  const document$5 = location$3.pipe(
    ...
    switchMap((url2) =>
      requestHTML(url2, { progress$: progress$2 }).pipe(
        catchError(() => {
  ...
  viewport$5.pipe(distinctUntilKeyChanged('offset'), debounceTime(100)).subscribe(({ offset }) => {
    history.replaceState(offset, '');
  });
  return document$5;
}
...
function setupSearchHighlighter(config6) {
  ...
  const highlight2 = (_, data, term) => {
    return ''.concat(data, '<mark data-md-highlight>').concat(term, '</mark>');
  };
  return (query) => {
    query = query.replace(/[\s*+\-:~^]+/g, ' ').trim();
    const match = new RegExp(
      '(^|'
        .concat(config6.separator, '|)(')
        .concat(query.replace(/[|\\{}()[\]^$+*?.-]/g, '\\$&').replace(separator, '|'), ')'),
      'img',
    );
    return (value) =>
      (0, import_escape_html2.default)(value)
        .replace(match, highlight2)
...
function setupSearchWorker(url2, index$2) {
  ...
  merge(of(location.protocol !== 'file:'), watchToggle('search'))
    .pipe(
      first((active) => active),
      switchMap(() => index$2),
    )
    .subscribe(({ config: config6, docs }) =>
      worker$.next({
        type: 0 /* SETUP */,
        data: {
          config: config6,
...
function selectedVersionCorrespondingURL(params) {
  var _a2;
  const { selectedVersionSitemap, selectedVersionBaseURL, currentLocation, currentBaseURL } =
    params;
  const current_path = (_a2 = safeURLParse(currentBaseURL)) == null ? void 0 : _a2.pathname;
...
function setupVersionSelector({ document$: document$5 }) {
  const config6 = configuration();
  const versions$ = requestJSON(new URL('../versions.json', config6.base)).pipe(
    catchError(() => EMPTY),
        ...
        (versions) =>
          new Map(
            versions.map((version) => [
              ''.concat(new URL('../'.concat(version.version, '/'), config6.base)),
              version,
            ]),
          ),
      ),
        ...
        fromEvent(document.body, 'click').pipe(
          ...
          switchMap((selectedVersionBaseURL) => {
            return fetchSitemap(selectedVersionBaseURL).pipe(
              map((sitemap) => {
                var _a2;
                return (_a2 = selectedVersionCorrespondingURL({
  ...
  document$5.pipe(switchMap(() => current$)).subscribe((current) => {
    ...
    if (outdated === null) {
      outdated = true;
      let ignored = ((_a2 = config6.version) == null ? void 0 : _a2.default) || 'latest';
    ...
    if (outdated) for (const warning of getComponentElements('outdated')) warning.hidden = false;
  });
}
...
function watchSearchQuery(el, { worker$ }) {
  ...
  if (searchParams.has('q')) {
    ...
    el.focus();
    watchToggle('search')
      .pipe(first((active) => !active))
      .subscribe(() => {
        const url2 = getLocation();
        url2.searchParams.delete('q');
        history.replaceState({}, '', ''.concat(url2));
      });
  }
...
function mountSearchQuery(el, { worker$ }) {
  const push$ = new Subject();
  const done$ = push$.pipe(ignoreElements(), endWith(true));
  combineLatest([worker$.pipe(first(isSearchReadyMessage)), push$], (_, query) => query)
    .pipe(distinctUntilKeyChanged('value'))
    .subscribe(({ value }) =>
      worker$.next({
        type: 2 /* QUERY */,
        data: value,
  ...
  push$.pipe(distinctUntilKeyChanged('focus')).subscribe(({ focus }) => {
    if (focus) setToggle('search', focus);
  });
...
function mountSearchResult(el, { worker$, query$ }) {
  ...
  watchToggle('search').subscribe((active) =>
    list.setAttribute('role', active ? 'list' : 'presentation'),
  );
  push$
    .pipe(withLatestFrom(query$), skipUntil(worker$.pipe(first(isSearchReadyMessage))))
  ...
  render$
    .pipe(
      mergeMap((item) => {
        ...
        if (typeof details === 'undefined') return EMPTY;
...
function mountSearchSuggest(el, { worker$, keyboard$: keyboard$2 }) {
  ...
  const query$ = merge(fromEvent(query, 'keydown'), fromEvent(query, 'focus')).pipe(
    ...
    .pipe(
      ...
      map(([{ suggest }, value]) => {
        ...
        if ((suggest == null ? void 0 : suggest.length) && words[words.length - 1]) {
          ...
          if (last2.startsWith(words[words.length - 1])) words[words.length - 1] = last2;
        } else {
          words.length = 0;
        }
        return words;
...
function mountSearch(el, { index$: index$2, keyboard$: keyboard$2 }) {
  ...
  try {
    ...
    fromEvent(el, 'click')
      .pipe(filter(({ target }) => target instanceof Element && !!target.closest('a')))
      ...
      switch (key.type) {
        ...
        case 'ArrowDown':
          if (typeof active === 'undefined') {
            ...
            const els = [
              query,
              ...getElements(':not(details) > [href], summary, details[open] [href]', result),
            ];
            const i = Math.max(
              0,
              (Math.max(0, els.indexOf(active)) + els.length + (key.type === 'ArrowUp' ? -1 : 1)) %
                els.length,
            );
            els[i].focus();
          }
        ...
        default:
          if (query !== getActiveElement()) query.focus();
      }
    });
    ...
    return merge(query$, mountSearchResult(result, { worker$, query$ })).pipe(
      mergeWith(
        ...getComponentElements('search-share', el).map((child) =>
          mountSearchShare(child, { query$ }),
        ),
        ...getComponentElements('search-suggest', el).map((child) =>
          mountSearchSuggest(child, { worker$, keyboard$: keyboard$2 }),
        ),
      ),
    );
  } catch (err) {
...
function mountSearchHiglight(el, { index$: index$2, location$: location$3 }) {
  return combineLatest([
    ...
    map(([index, url2]) => setupSearchHighlighter(index.config)(url2.searchParams.get('h'))),
    map((fn) => {
      var _a2;
      ...
      for (let node = it.nextNode(); node; node = it.nextNode()) {
        if ((_a2 = node.parentElement) == null ? void 0 : _a2.offsetHeight) {
          ...
          if (replaced.length > original.length) nodes.set(node, replaced);
        }
      }
...
function mountSidebar(el, { header$: header$3, ...options }) {
  ...
  return defer(() => {
    const push$ = new Subject();
    const done$ = push$.pipe(ignoreElements(), endWith(true));
    const next$ = push$.pipe(auditTime(0, animationFrameScheduler));
    next$.pipe(withLatestFrom(header$3)).subscribe({
      /* Handle emission */
      next([{ height }, { height: offset }]) {
        inner.style.height = ''.concat(height - 2 * y, 'px');
        el.style.top = ''.concat(offset, 'px');
      },
      /* Handle complete */
      complete() {
        inner.style.height = '';
    ...
    from(getElements('label[tabindex]', el))
      .pipe(
        mergeMap((label) =>
          fromEvent(label, 'click').pipe(
            observeOn(asyncScheduler),
            map(() => label),
            takeUntil(done$),
          ),
      ...
      .subscribe((label) => {
        const input = getElement('[id="'.concat(label.htmlFor, '"]'));
        const nav2 = getElement('[aria-labelledby="'.concat(label.id, '"]'));
        nav2.setAttribute('aria-expanded', ''.concat(input.checked));
      });
...
function fetchSourceFactsFromGitHub(user, repo) {
  if (typeof repo !== 'undefined') {
    const url2 = 'https://api.github.com/repos/'.concat(user, '/').concat(repo);
    return zip(
      /* Fetch version */
      requestJSON(''.concat(url2, '/releases/latest')).pipe(
        catchError(() => EMPTY),
        // @todo refactor instant loading
        map((release) => ({
    ...
    ).pipe(map(([release, info]) => ({ ...release, ...info })));
  } else {
    const url2 = 'https://api.github.com/users/'.concat(user);
...
function fetchSourceFactsFromGitLab(base, project) {
  const url2 = 'https://'.concat(base, '/api/v4/projects/').concat(encodeURIComponent(project));
  return zip(
    /* Fetch version */
    requestJSON(''.concat(url2, '/releases/permalink/latest')).pipe(
      catchError(() => EMPTY),
      // @todo refactor instant loading
      map(({ tag_name }) => ({
        version: tag_name,
    ...
    (fetch$ = defer(() => {
      ...
      if (cached) {
        ...
        if (els.length) {
          ...
          if (!(consent && consent.github)) return EMPTY;
        }
...
function mountTabs(el, options) {
  return defer(() => {
    ...
    return (
      feature('navigation.tabs.sticky') ? of({ hidden: false }) : watchTabs(el, options)
    ).pipe(
      tap((state) => push$.next(state)),
      finalize(() => push$.complete()),
      map((state) => ({ ref: el, ...state })),
...
function watchTableOfContents(el, { viewport$: viewport$5, header$: header$3 }) {
  ...
  for (const anchor of anchors) {
    ...
    const target = getOptionalElement('[id="'.concat(id, '"]'));
      ...
      defer(() => {
        ...
        return of(
          [...table].reduce((index, [anchor, target]) => {
            ...
            for (; parent; parent = parent.offsetParent) offset += parent.offsetTop;
        ...
        switchMap(([index, adjust]) =>
          viewport$5.pipe(
            scan(
              ([prev, next], { offset: { y }, size }) => {
...
function mountTableOfContents(
  el,
  { viewport$: viewport$5, header$: header$3, main$: main$2, target$: target$2 },
) {
  return defer(() => {
    const push$ = new Subject();
    const done$ = push$.pipe(ignoreElements(), endWith(true));
...
function watchBackToTop(_el, { viewport$: viewport$5, main$: main$2, target$: target$2 }) {
  ...
  const active$ = main$2.pipe(map(({ active }) => active));
...
function mountBackToTop(
  ...
  header$3.pipe(takeUntil(done$), distinctUntilKeyChanged('height')).subscribe(({ height }) => {
    el.style.top = ''.concat(height + 16, 'px');
  });
...
function patchIndeterminate({ document$: document$5, tablet$: tablet$2 }) {
  document$5
    .pipe(
      switchMap(() => getElements('.md-toggle--indeterminate')),
      tap((el) => {
...
function patchScrollfix({ document$: document$5 }) {
  document$5
    .pipe(
      ...
      mergeMap((el) => fromEvent(el, 'touchstart').pipe(map(() => el))),
    )
    .subscribe((el) => {
...
function patchScrolllock({ viewport$: viewport$5, tablet$: tablet$2 }) {
  combineLatest([watchToggle('search'), tablet$2])
    .pipe(
      map(([active, tablet]) => active && !tablet),
      switchMap((active) => of(active).pipe(delay(active ? 400 : 100))),
...
if (!Object.entries)
  Object.entries = function (obj) {
    const data = [];
    for (const key of Object.keys(obj)) data.push([key, obj[key]]);
    return data;
  };
if (!Object.values)
...
if (typeof Element !== 'undefined') {
  ...
  if (!Element.prototype.replaceWith)
    Element.prototype.replaceWith = function (...nodes) {
      ...
      if (parent) {
        if (nodes.length === 0) parent.removeChild(this);
...
function fetchSearchIndex() {
  if (location.protocol === 'file:') {
    return watchScript(''.concat(new URL('search/search_index.js', config5.base))).pipe(
      // @ts-ignore - @todo fix typings
      map(() => __index),
      shareReplay(1),
    );
  } else {
    return requestJSON(new URL('search/search_index.json', config5.base));
  }
}
...
keyboard$.pipe(filter(({ mode }) => mode === 'global')).subscribe((key) => {
  switch (key.type) {
    ...
    case ',':
      ...
      if (typeof prev !== 'undefined') setLocation(prev);
      break;
    /* Go to next page */
    case 'n':
    case '.':
      const next = getOptionalElement('link[rel=next]');
      if (typeof next !== 'undefined') setLocation(next);
      break;
...
var content$ = defer(() =>
  merge(
    ...
    ...getComponentElements('content').map((el) =>
      mountContent(el, { viewport$: viewport$2, target$, print$ }),
    ),
    ...getComponentElements('content').map((el) =>
      feature('search.highlight') ? mountSearchHiglight(el, { index$, location$ }) : EMPTY,
    ),
    ...getComponentElements('header').map((el) =>
      mountHeader(el, { viewport$: viewport$2, header$, main$ }),
    ),
    ...getComponentElements('header-title').map((el) =>
      mountHeaderTitle(el, { viewport$: viewport$2, header$ }),
    ),
    ...getComponentElements('sidebar').map((el) =>
      el.getAttribute('data-md-type') === 'navigation'
        ? at(screen$, () => mountSidebar(el, { viewport$: viewport$2, header$, main$ }))
        : at(tablet$, () => mountSidebar(el, { viewport$: viewport$2, header$, main$ })),
    ),
    ...
    ...getComponentElements('top').map((el) =>
      mountBackToTop(el, { viewport$: viewport$2, header$, main$, target$ }),
    ),
  ),
);
...
var isPageVisible$ = fromEvent(document, 'visibilitychange').pipe(
...
var header$2 = watchHeader2(getComponentElement('header'), { viewport$: viewport$3 });
...
var mapLocation = (ev) => {
  const eventMap = {
    beforeunload:
      ev.target && ev.target instanceof HTMLAnchorElement ? new URL(ev.target.href) : getLocation(),
...
var navigationEvents$ =
  'navigation' in customWindow
    ...
    : merge(
        ...
        location$2.pipe(distinctUntilChanged()),
...
function watchLicenseHash() {
  return combineLatest([
    ...
    filter(([first2, second]) => first2.pathname === second.pathname),
    map(([_, second]) => second),
    filter((url2) => isLicenseHash(url2)),
    tap(handleLicenseHash),
  );
}
function isHidden2({ viewport$: viewport$5 }) {
  if (!feature('header.autohide')) {
    return of(false);
  }
  const header2 = getComponentElement('header');
  if (!header2 || !viewport$5) {
    return of(false);
  }
  if (
    header2.checkVisibility({
...
function watchHeader2(el, options) {
  return defer(() => combineLatest([watchElementSize(el), isHidden2(options)])).pipe(
    map(([{ height }, hidden]) => ({
      height,
      hidden,
    })),
...
var header = getComponentElement('header');
var nav = getComponentElement('tabs');
var footer = document.querySelector('#hero-footer');

// src/assets/javascripts/utils/fetchWorker.ts
async function locateMissingWorker() {
  const origin2 = window.location.origin || import.meta.url.replace('docs', '');
  const metaFile = new URL('manifest.json', origin2);
  const response = await fetch(metaFile);
  if (response.ok) {
    const meta = await response.json();
    const cacheWorker = meta.find(
      ([key, _value]) => key.includes('cacheWorker') && !key.includes('.map'),
    );
    if (!cacheWorker) {
...
var cacheWorkerUrl = 'cacheWorker.T55XCAMX.js';
if ('serviceWorker' in navigator && window.isSecureContext) {
  ...
  const register2 = async () => {
    navigator.serviceWorker
      ...
      .then((registration) => {
        if (registration == null ? void 0 : registration.installing) {
          logger.debug('Service worker installing');
        } else if (registration == null ? void 0 : registration.waiting) {
          logger.debug('Service worker installed');
        } else if (registration == null ? void 0 : registration.active) {
...
var _clean = function _clean2(value, index, target, targets) {
  ...
  if (typeof value !== 'object') {
    ...
    for (p in value) {
      result[p] =
        p !== 'onAutoKill' && _isFunction3(value[p]) ? value[p](index, target, targets) : value[p];
    }
    return result;
  }
};
  ...
  var rect = element.getBoundingClientRect(),
    ...
    cRect = isRoot
      ? {
          ...
          left:
            _docEl.clientLeft - (_window.pageXOffset || _docEl.scrollLeft || _body.scrollLeft || 0),
        }
...
var _getGSAP3 = function _getGSAP4() {
  return (
    gsap3 ||
    (typeof window !== 'undefined' && (gsap3 = window.gsap) && gsap3.registerPlugin && gsap3)
  );
};
var _startup = 1;
var _observers = [];
...
var _horizontal = {
  ...
  sc: _scrollCacheFunc(function (value) {
    return arguments.length
      ...
      : _win3.pageXOffset || _doc3[_scrollLeft] || _docEl2[_scrollLeft] || _body2[_scrollLeft] || 0;
  }),
};
var _vertical = {
  s: _scrollTop,
  ...
  sc: _scrollCacheFunc(function (value) {
    return arguments.length
      ...
      : _win3.pageYOffset || _doc3[_scrollTop] || _docEl2[_scrollTop] || _body2[_scrollTop] || 0;
  }),
};
var _getTarget = function _getTarget2(t, self) {
  return (
    ((self && self._ctx && self._ctx.selector) || gsap3.utils.toArray)(t)[0] ||
    (typeof t === 'string' && gsap3.config().nullTargetWarn !== false
  ...
  var v1 = value,
    ...
    getVelocity = function getVelocity2(latestValue) {
      ...
      return t1 === t2 || t - t2 > dropToZeroTime
        ? 0
        : ((v1 + (useDelta ? vOld : -vOld)) / ((useDelta ? t : t1) - tOld)) * 1e3;
    };
  return {
    update,
    reset,
...
var _initCore5 = function _initCore6(core) {
  ...
  if (!_coreInitted3 && gsap3 && typeof document !== 'undefined' && document.body) {
    ...
    _isTouch = Observer.isTouch =
      _win3.matchMedia && _win3.matchMedia('(hover: none), (pointer: coarse)').matches
        ? 1
        : 'ontouchstart' in _win3 || navigator.maxTouchPoints > 0 || navigator.msMaxTouchPoints > 0
          ? 2
          : 0;
    _eventTypes = Observer.eventTypes = (
      'ontouchstart' in _docEl2
        ? 'touchstart,touchmove,touchcancel,touchend'
        : !('onpointerdown' in _docEl2)
...
var Observer = /* @__PURE__ */ (function () {
  ...
  _proto.init = function init5(vars) {
    ...
    var tolerance = vars.tolerance,
      ...
      preventDefault2 = vars.preventDefault,
      onStop = vars.onStop,
    ...
    var id,
      ...
      prevDeltaX = 0,
      prevDeltaY = 0,
      passive = vars.passive || (!preventDefault2 && vars.passive !== false),
      ...
      _onWheel = function _onWheel2(e) {
        ...
        var multiplier =
          (e.deltaMode === 1 ? lineHeight : e.deltaMode === 2 ? _win3.innerHeight : 1) * wheelSpeed;
...
var _keepClamp = function _keepClamp2(value, clamp3) {
  return clamp3 && (!_isString5(value) || value.substr(0, 6) !== 'clamp(')
    ? 'clamp(' + value + ')'
    : value;
};
...
var _getViewportDimension = function _getViewportDimension2(dimensionProperty) {
  return (
    (dimensionProperty === 'Height' ? _100vh : _win4['inner' + dimensionProperty]) ||
    _docEl3['client' + dimensionProperty] ||
    _body3['client' + dimensionProperty]
  );
};
...
var _maxScroll = function _maxScroll2(element, _ref2) {
  ...
  return Math.max(
    ...
    (s = 'scroll' + d2) && (a = _getProxyProp(element, s))
      ...
      : _isViewport3(element)
        ? (_docEl3[s] || _body3[s]) - _getViewportDimension(d2)
...
var _getBounds = function _getBounds2(element, withoutTransforms) {
  var tween =
      withoutTransforms &&
      _getComputedStyle(element)[_transformProp2] !== 'matrix(1, 0, 0, 1, 0, 0)' &&
      gsap4
        .to(element, {
          x: 0,
          y: 0,
...
var _createMarker = function _createMarker2(
  ...
  var e = _doc4.createElement('div'),
    ...
    parent = useFixedPosition ? _body3 : container,
  ...
  css +=
    'position:' + ((isScroller || containerAnimation) && useFixedPosition ? 'fixed;' : 'absolute;');
...
var _sync = function _sync2() {
  return (
    _getTime2() - _lastScrollTime > 34 && (_rafID || (_rafID = requestAnimationFrame(_updateAll)))
  );
};
...
var _onResize = function _onResize2(force) {
  ...
  (force === true ||
    (!_refreshing &&
      ...
      (!_ignoreMobileResize ||
        _baseScreenWidth !== _win4.innerWidth ||
        Math.abs(_win4.innerHeight - _baseScreenHeight) > _win4.innerHeight * 0.25))) &&
...
var _clearScrollMemory = function _clearScrollMemory2(scrollRestoration, force) {
  ...
  _isString5(scrollRestoration) &&
    (_win4.history.scrollRestoration = _scrollRestoration = scrollRestoration);
};
var _refreshingAll;
var _refreshID = 0;
var _queueRefreshID;
...
var _parsePosition3 = function _parsePosition4(
  ...
  if (marker) {
    ...
    _positionMarker(
      ...
      (isStart && position > 20) ||
        (!isStart &&
          (useFixedPosition ? Math.max(_body3[p1], _docEl3[p1]) : marker.parentNode[p1]) <=
            position + 1),
    );
    if (useFixedPosition) {
...
var ScrollTrigger3 = /* @__PURE__ */ (function () {
  ...
  _proto.init = function init5(vars, animation) {
    ...
    vars = _setDefaults3(
      _isString5(vars) || _isNumber3(vars) || vars.nodeType
        ? {
            trigger: vars,
          }
        : vars,
    ...
    if (snap3) {
      ...
      'scrollBehavior' in _body3.style &&
        gsap4.set(isViewport ? [_body3, _docEl3] : scroller, {
          scrollBehavior: 'auto',
        });
      _scrollers.forEach(function (o) {
        return (
          _isFunction5(o) &&
          o.target === (isViewport ? _doc4.scrollingElement || _docEl3 : scroller) &&
          (o.smooth = false)
        );
      });
      snapFunc = _isFunction5(snap3.snapTo)
        ? snap3.snapTo
        : snap3.snapTo === 'labels'
          ...
          if (
            ...
            var progress = (scroll - start) / change,
              ...
              velocity = refreshedRecently
                ? 0
                : ((totalProgress - snap22) / (_getTime2() - _time2)) * 1e3 || 0,
              change1 = gsap4.utils.clamp(
                -progress,
                1 - progress,
    ...
    if (markers) {
      ...
      containerAnimation &&
        (caMarkerSetter = gsap4.quickSetter([markerStart, markerEnd], direction.a, _px));
      if (
        !useFixedPosition &&
    ...
    self.refresh = function (soft, force, position, pinOffset) {
      ...
      if (_isString5(parsedEnd) && !parsedEnd.indexOf('+=')) {
        if (~parsedEnd.indexOf(' ')) {
          ...
          parsedEnd = _isString5(parsedStart)
            ? parsedStart
            : (containerAnimation
                ? gsap4.utils.mapRange(
                    0,
      ...
      if (pin && !(_clampingMax && self.end >= _maxScroll(scroller, direction))) {
        ...
        if (!max && end > 1) {
          ...
          if (
            isViewport &&
            _getComputedStyle(_body3)['overflow' + direction.a.toUpperCase()] !== 'scroll'
          ) {
        ...
        animation &&
          ...
          animation.totalProgress(
            containerAnimation && start < -1e-3 && !prevProgress
              ? gsap4.utils.normalize(start, end, 0)
              : prevProgress,
            true,
          );
        self.progress =
    ...
    self.update = function (reset, recordVelocity, forceFake) {
      ...
      if (anticipatePin && pin && !_refreshing && !_startup2 && _lastScrollTime) {
        if (
          !clipped &&
          start < scroll + ((scroll - scroll2) / (_getTime2() - _time2)) * anticipatePin
        ) {
          clipped = 1e-4;
        } else if (
          clipped === 1 &&
          end > scroll + ((scroll - scroll2) / (_getTime2() - _time2)) * anticipatePin
        ) {
          clipped = 0.9999;
        }
      }
      if (clipped !== prevProgress2 && self.enabled) {
        ...
        if (stateChanged && !_refreshing) {
          toggleState =
            clipped && !prevProgress2 ? 0 : clipped === 1 ? 1 : prevProgress2 === 1 ? 2 : 3;
          if (isToggle) {
            action =
              (!toggled &&
        ...
        toggleClass &&
          ...
          _toArray2(toggleClass.targets).forEach(function (el) {
        ...
        if (stateChanged && !_refreshing) {
          ...
          if (
            fastScrollEnd &&
            !isActive &&
            Math.abs(self.getVelocity()) > (_isNumber3(fastScrollEnd) ? fastScrollEnd : 2500)
          ) {
            _endAnimation(self.callbackAnimation);
            scrubTween
              ? scrubTween.progress(1)
  ...
  ScrollTrigger4.enable = function enable() {
    ...
    if (gsap4) {
      ...
      if (_body3) {
        ...
        var bodyHasStyle = _body3.hasAttribute('style'),
          bodyStyle = _body3.style,
          border = bodyStyle.borderTopStyle,
          AnimationProto = gsap4.core.Animation.prototype,
          bounds,
          i;
  ...
  ScrollTrigger4.config = function config6(vars) {
    ...
    'ignoreMobileResize' in vars &&
      (_ignoreMobileResize = ScrollTrigger4.isTouch === 1 && vars.ignoreMobileResize);
  ...
  ScrollTrigger4.scrollerProxy = function scrollerProxy(target, vars) {
    ...
    if (vars) {
      isViewport
        ? _proxies.unshift(_win4, vars, _body3, vars, _docEl3, vars)
  ...
  ScrollTrigger4.positionInViewport = function positionInViewport(
    element,
    referencePoint,
    horizontal,
  ) {
...
ScrollTrigger3.refresh = function (safe) {
  return safe ? _onResize(true) : (_coreInitted4 || ScrollTrigger3.register()) && _refreshAll(true);
};
...
ScrollTrigger3.batch = function (targets, vars) {
  ...
  for (p in vars) {
    varsCopy[p] =
      p.substr(0, 2) === 'on' && _isFunction5(vars[p]) && p !== 'onRefreshInit'
  ...
  _toArray2(targets).forEach(function (target) {
    ...
    result.push(ScrollTrigger3.create(config6));
  });
  return result;
};
...
var _getScrollNormalizer = function _getScrollNormalizer2(vars) {
  ...
  var _vars2 = vars,
    ...
    wheelRefresh = 0,
    resolveMomentumDuration = _isFunction5(momentum)
      ? function () {
  ...
  if (feedbackForm && feedbackForm instanceof HTMLFormElement) {
    return fromEvent(feedbackForm, 'submit').pipe(
      ...
      tap((ev) => {
        ...
        const note = feedbackForm.querySelector(
          ".md-feedback__note [data-md-value='".concat(data, "']"),
        );
...
var HeroStore = class _HeroStore {
  ...
  static getInstance() {
    var _a2;
    (_a2 = _HeroStore.instance) != null ? _a2 : (_HeroStore.instance = new _HeroStore());
    return _HeroStore.instance;
  }
  /**
  ...
  updateState(update, component) {
    ...
    const changes = Object.entries(update).filter(
      ([key, value]) =>
        value !== null && key in this.state$.value && this.state$.value[key] !== value,
    );
    if (changes.length === 0) {
      return;
    }
  ...
  createObserver(name, updateFn, component) {
    return {
      next: (value) => {
        logger.debug('Observer '.concat(name, ' received:'));
        logger.table(value);
  ...
  initSubscriptions() {
    ...
    const landing$ = combineLatest([
      isPartiallyInViewport(document.getElementById('parallax-hero-image-layer')),
      this.section$.pipe(filter((section) => section < 1 /* Impact */)),
    ]).pipe(
    ...
    const header$3 = watchHeader2(getComponentElement('header'), { viewport$: viewport$4 });
    const tab$ = watchTabs(getComponentElement('tabs'), {
      viewport$: viewport$4,
      header$: header$3,
    });
    const headerWatch$ = combineLatest([header$3, tab$]).pipe(
      tap(([header2, tabs]) => {
        if (tabs == null ? void 0 : tabs.hidden) {
          setCssVariable('--header-height', header2.hidden ? '0' : ''.concat(header2.height, 'px'));
        } else {
          ...
          setCssVariable(
            ...
            header2.hidden
              ? ''.concat(tabHeight, 'px')
              : ''.concat(header2.height + tabHeight, 'px'),
          );
        }
      }),
      map(([header2, _tabs]) => header2),
      tap(this.createObserver('header$', (header2) => ({ header: header2 }))),
    );
    const parallax$ = combineLatest([
      viewport$4,
      watchHeader2(getComponentElement('header'), { viewport$: viewport$4 }),
      watchMediaQuery('(orientation: portrait)'),
    ]).pipe(
      map(([viewport, header2, portrait]) => {
        return {
    ...
    const video$ = this.getVideoState$((canPlay) => {
      if (canPlay && 'canPlay' in canPlay) {
        this.videoState$.next(canPlay);
      }
    });
    const transition$ = this.getTransitionState$((t) => {
      this.transitionState$.next(t);
    });
    const loc$ = navigationEvents$.pipe(
      tap(
        this.createObserver('location$', (location2) => ({
          location: location2,
        })),
      ),
    );
   ...
   */
  getStateObservable(observerFunc, key, observableName, predicate) {
  ...
  getVideoState$(observerFunc) {
    return this.state$.pipe(
      map((state) => predicates.videoPredicate(state).canPlay),
  ...
  getTransitionState$(observerFunc) {
  ...
  debugStateChange(updates) {
    if (weAreDev) {
      ...
      for (const [name, predicate] of Object.entries(predicates).filter(
        ...
        logger.debug(''.concat(name, ':'), predicate({ ...oldState, ...updates }));
      }
    }
  }
  /**
...
function getContentElements(element) {
  ...
  const hasExcludedClass = (el) => excludedClasses.some((cls) => el.classList.contains(cls));
  const hasContent = (el) => {
    var _a2, _b, _c;
    return (
      ((_c =
        (_b = (_a2 = el.textContent) == null ? void 0 : _a2.trim()) == null ? void 0 : _b.length) !=
      null
        ? _c
        : 0) > 0 || el.querySelector('img, svg, video, picture') !== null
    );
  };
  return gsapWithCSS.utils
    .toArray('*', element)
    .filter((el) => el instanceof Element && el !== element && isValidElement(el, element))
...
var hide = (vars = {}) => {
  return {
    ...
    visibility: 'visible',
    contentVisibility:
      vars['contentVisibility'] && vars['contentVisibility'] === 'visible' ? 'visible' : void 0,
  };
};
var show = (vars = {}) => {
  return {
...
function getFadeVars(out, yPercent, direction) {
...
gsapWithCSS.registerEffect({
  ...
  effect: (targets, config6) => {
    ...
    return gsapWithCSS
      ...
      .add(gsapWithCSS.set(targets, { zIndex: 0, ...hide() }))
      .add(gsapWithCSS.to(section.bg, { yPercent: -15 * dFactor, zIndex: 0, opacity: 0 }))
      .add(
        gsapWithCSS.set([section.outerWrapper, section.innerWrapper], {
          zIndex: -1,
          ...show(),
        }),
      )
...
var fade = (
  ...
  var _a2, _b, _c;
  logger.debug('fade: targets: '.concat(logObject(targets, 'fade target')));
  ...
  if (fromConfig && toConfig) {
    fadeVars = getFadeVars(
      out || false,
      Number.parseInt(
        ((_a2 = fromConfig.yPercent) == null ? void 0 : _a2.toString()) || '50',
        10,
      ) ||
        Number.parseInt(
          ((_b = toConfig == null ? void 0 : toConfig.yPercent) == null ? void 0 : _b.toString()) ||
            '0',
          10,
        ) ||
        50,
    ...
    fadeVars = getFadeVars(
      out || false,
      Number.parseInt(
        ((_c = OBSERVER_CONFIG.fades.fadeInConfig.normal.from.yPercent) == null
          ? void 0
          : _c.toString()) || '50',
        10,
      ),
  ...
  effect: (targets, config6) => {
    ...
    logger.debug(
      'transitionSection: direction: '
        .concat(direction, ', section: ')
        .concat(console.dir(section)),
    );
    const dFactor = getDFactor(direction);
    ...
    const header2 = gsapWithCSS.utils.toArray(OBSERVER_CONFIG.header);
    const revealHeader = gsapWithCSS.timeline();
    if (header2.length > 0 && index === 0) {
      logger.debug('Revealing header');
      revealHeader
        .add(() => {
          for (const el of header2) {
      ...
      logger.debug('header classlists: '.concat(logObject(header2, 'header classlist')));
    }
    const leaving = index === 1 && 1 /* Down */;
    if (index === 0 || leaving) {
      gsapWithCSS.to('.cta__container', {
    ...
    return gsapWithCSS
      ...
      .add(gsapWithCSS.to(section.content, { autoAlpha: 1, zIndex: 1 }), 'fadeIn+=0.3')
      .add(revealHeader, '>');
  },
});
gsapWithCSS.registerEffect({
  ...
  extendTimeline: true,
  effect: (targets, config6) => {
    const combinedConfig = { ...OBSERVER_CONFIG.fades, ...config6 };
    const { direction, fromConfig, toConfig } = combinedConfig;
    const checkedTargets = Array.isArray(targets) ? targets : [targets];
    logger.debug('fading in targets: '.concat(logObject(checkedTargets, 'fadeIn target')));
...
gsapWithCSS.registerEffect({
  ...
  effect: (targets, config6) => {
    const combinedConfig = { ...OBSERVER_CONFIG.fades, ...config6 };
    const checkedTargets = Array.isArray(targets) ? targets : [targets];
    logger.debug('fadeOut: targets: '.concat(logObject(checkedTargets, 'fadeOut target')));
    const { direction, fromConfig, toConfig } = combinedConfig;
    return fade(checkedTargets, { out: true, direction, fromConfig, toConfig });
  },
});
var blink = (targets, config6 = {}) => {
  logger.debug('blink: targets: '.concat(logObject(targets, 'blink target')));
...
var jump = (targets, config6 = {}) => {
  logger.debug('jump: targets: '.concat(logObject(targets, 'jump target')));
...
var scaleUp = (targets, config6 = {}) => {
  logger.debug('scaleUp: targets: '.concat(logObject(targets, 'scaleUp target')));
  return gsapWithCSS.to(targets, {
    scale: 1.5,
    ease: 'elastic',
    duration: 0.5,
...
function animateLogo(scope, config6) {
  ...
  gsapWithCSS
    .matchMedia()
    .add({ reducedMotion: '(prefers-reduced-motion: reduce)' }, (context4) => {
      const { reducedMotion } = context4.conditions;
...
function animateText(scope, targets, config6) {
  ...
  if (!innerContainer || !containerTarget || !msgFrag) {
    logger.error(
      'Needed element(s) not found for message animation. \n  msgFrag: '
        .concat(logObject(msgFrag), ', \n  innerContainer: ')
        .concat(logObject(innerContainer), ', \n  containerTarget: ')
        .concat(logObject(containerTarget)),
    );
    return gsapWithCSS.timeline();
  }
  ...
  if (config6.sharedVars && Object.keys(config6.sharedVars).length) {
    config6.fromVars =
      config6.fromVars && Object.keys(config6.fromVars).length
        ? { ...config6.fromVars, ...config6.sharedVars }
        : config6.sharedVars;
    config6.toVars =
      config6.toVars && Object.keys(config6.toVars).length
        ? { ...config6.toVars, ...config6.sharedVars }
        : config6.sharedVars;
  }
  const fromVars = config6.fromVars || {};
  const toVars = config6.toVars || {};
  gsapWithCSS
    .matchMedia()
    .add({ reducedMotion: '(prefers-reduced-motion: reduce)' }, (context4) => {
      const { reducedMotion } = context4.conditions;
  ...
  return messageTimeline
    ...
    .add(animateLogo(scope, config6), '<=2.5')
    .add(() => logger.debug('Finishing text animation'), '>')
    .add(gsapWithCSS.set([containerTarget, innerContainer], hide()), '>');
}
gsapWithCSS.registerEffect({
  name: 'animateMessage',
  extendTimeline: true,
  ...
  effect: (targets, config6) => {
    var _a2, _b, _c;
    const scope =
      ((_a2 = config6.sharedVars) == null ? void 0 : _a2.callbackScope) ||
      config6.callbackScope || // biome-ignore lint/complexity/useLiteralKeys: This clarifies that these are not standard API properties
      ((_b = config6.sharedVars) == null ? void 0 : _b['defaults']['callbackScope']) ||
      ((_c = config6.toVars) == null ? void 0 : _c.callbackScope);
...
var HeroObservation = class _HeroObservation {
  constructor() {
    ...
    this.currentIndex = -1 /* NotInitialized */;
  ...
  static getInstance() {
    var _a2;
    return (_a2 = _HeroObservation.instance) != null
      ? _a2
      : (_HeroObservation.instance = new _HeroObservation());
  }
  ...
  setupSubscriptions() {
    ...
    const index$2 = this.indexSubject.asObservable().pipe(
      distinctUntilChanged((a, b) => a === b),
      skipUntil(atHome$),
      distinctUntilChanged((a, b) => a === b),
    );
    ...
    this.subscriptions.add(index$2.subscribe((index) => this.store.section$.next(index)));
  }
  updateIndex(index) {
  ...
  onLoad() {
    ...
    (_a2 = this.header) == null
      ? void 0
      : _a2.forEach((header2) => {
          if (header2 instanceof HTMLElement && header2.style.opacity.length) {
    ...
    this.subscriptions.add(
      navigationEvents$.pipe(filter((url2) => !isHome(url2))).subscribe(() => {
        var _a3, _b;
        (_a3 = this.header) == null
          ? void 0
          : _a3.forEach((header2) => {
              if ('hidden' in header2 && header2.hidden) {
                header2.hidden = false;
              }
              if (header2 instanceof HTMLElement && header2.style.background.length) {
                header2.style.background = '';
              }
            });
        (_b = this.footer) == null ? void 0 : _b.removeAttribute('hidden');
        gsapWithCSS.set(this.footer, { autoAlpha: 1, opacity: 1 });
        gsapWithCSS.set(this.header, { autoAlpha: 1, opacity: 1 });
      }),
    );
    ...
    requestAnimationFrame(() => {
      ...
      this.sections.forEach((section) => {
        gsapWithCSS.set(section.element, { autoAlpha: 0 });
      });
    ...
    if (!this.initialized) {
      ...
      if (sectionTarget) {
        ...
        logger.warn('Target section '.concat(target.id, ' not found in sections'));
      }
    }
  }
  /**
  ...
  setupSections() {
    ...
    this.sections = sectionEls.map((el, index) => {
      ...
      logger.debug('Setting up section '.concat(index), {
        outerWrapper,
        innerWrapper,
        bg,
        content,
      });
      const animation = gsapWithCSS
        .timeline({
          ...this.defaultTimelineVars,
          defaults: {
            paused: true,
  ...
  async transition(direction, scenicRoute) {
    ...
    if (!scenicRoute) {
    ...
    } else if (scenicRoute && direction === 1 /* Down */) {
      this.goToSection(nextIndex, direction);
      ...
      while (remainingSections > 0) {
        ...
        if (this.currentIndex !== this.sectionIndexLength && this.currentIndex === nextIndex) {
          this.goToSection(nextIndex + 1 /* Down */, direction);
          nextIndex++;
          remainingSections--;
        } else {
          this.animating = false;
  ...
  constructTransitionTimeline(direction, index, tl) {
    ...
    logger.debug(
      'Timeline state: currentIndex='
        .concat(this.currentIndex, ', targetIndex=')
        .concat(index, ', direction=')
        .concat(direction, ', sectionsCount=')
        .concat(this.sectionCount),
    );
  ...
  goToSection(index, direction) {
    ...
    const clampedIndex = gsapWithCSS.utils.clamp(0, this.sections.length - 1, index);
    ...
    if (!this.sections[clampedIndex].animation) {
      throw new Error(
        'No animation found for section '.concat(clampedIndex, ' -- failed to setup transition'),
      );
    }
    logger.debug(
      'Transitioning to section '.concat(clampedIndex, ' in direction ').concat(direction),
    );
  ...
  onStartFunction(index) {
    return () => {
      this.store.updateHeroState({ isTransitioning: true });
      this.animating = true;
      this.updateIndex(index);
      logger.info('Transitioning to section '.concat(index));
    };
  }
  onCompleteFunction(index) {
    return () => {
      this.store.updateHeroState({ isTransitioning: false });
      this.animating = false;
      logger.info('Transition to section '.concat(index, ' complete'));
      this.updateIndex(index);
    };
  }
  onActionFunction(direction, scenicRoute = false) {
    return () => {
      if (!this.animating) {
        this.transition(direction, scenicRoute);
        logger.info(
          'Action function triggered - direction: '
            .concat(direction, ', scenicRoute: ')
            .concat(scenicRoute),
        );
      } else {
  ...
  onHoverFunction(element) {
    return () => !this.animating && gsapWithCSS.to(element, { autoAlpha: 1, duration: 0.3 });
  }
  onHoverEndFunction(element) {
    return () => !this.animating && gsapWithCSS.to(element, { autoAlpha: 0, duration: 0.3 });
  }
  createHoverObserver(element) {
    return Observer.create({
      type: 'hover',
      target: element,
  ...
  setupObserver() {
    ...
    this.clickObserver = Observer.create({
      ...
      onClick: this.onActionFunction(1 /* Down */, true),
      onRelease: this.onActionFunction(1 /* Down */, true),
      preventDefault: true,
    });
...
var svgLogo = 'assets/images/logo_only_color_transp.WJCCEJU7.svg';
var breakFreeAv11280 = 'assets/videos/hero/break_free/break_free_av1_1280.RSEG4CUZ.webm';
var breakFreeAv11920 = 'assets/videos/hero/break_free/break_free_av1_1920.N4GUZFCE.webm';
var breakFreeAv12560 = 'assets/videos/hero/break_free/break_free_av1_2560.MIKJMNDK.webm';
var breakFreeAv13840 = 'assets/videos/hero/break_free/break_free_av1_3840.TPJHXYML.webm';
var breakFreeAv1426 = 'assets/videos/hero/break_free/break_free_av1_426.J3IUQ25H.webm';
var breakFreeAv1640 = 'assets/videos/hero/break_free/break_free_av1_640.KCOIXYDS.webm';
var breakFreeAv1854 = 'assets/videos/hero/break_free/break_free_av1_854.VUEDJ3DP.webm';
var breakFreeH2641280 = 'assets/videos/hero/break_free/break_free_h264_1280.KARTASAH.mp4';
var breakFreeH2641920 = 'assets/videos/hero/break_free/break_free_h264_1920.IG6ELZDZ.mp4';
var breakFreeH2642560 = 'assets/videos/hero/break_free/break_free_h264_2560.RUNGB3VA.mp4';
var breakFreeH2643840 = 'assets/videos/hero/break_free/break_free_h264_3840.MG5X5NJZ.mp4';
var breakFreeH264426 = 'assets/videos/hero/break_free/break_free_h264_426.RI7JWYYA.mp4';
var breakFreeH264640 = 'assets/videos/hero/break_free/break_free_h264_640.EOYTCEFU.mp4';
var breakFreeH264854 = 'assets/videos/hero/break_free/break_free_h264_854.YOJRKDM6.mp4';
var breakFreeVp91280 = 'assets/videos/hero/break_free/break_free_vp9_1280.XO7Q6WAK.webm';
var breakFreeVp91920 = 'assets/videos/hero/break_free/break_free_vp9_1920.YRJW4KBB.webm';
var breakFreeVp92560 = 'assets/videos/hero/break_free/break_free_vp9_2560.RIK2IL3V.webm';
var breakFreeVp93840 = 'assets/videos/hero/break_free/break_free_vp9_3840.7FNPDD33.webm';
var breakFreeVp9426 = 'assets/videos/hero/break_free/break_free_vp9_426.DWSTPWJ6.webm';
var breakFreeVp9640 = 'assets/videos/hero/break_free/break_free_vp9_640.VJFDHXNY.webm';
var breakFreeVp9854 = 'assets/videos/hero/break_free/break_free_vp9_854.KEEGXMBB.webm';
var breakFreeAvif1280 = 'assets/videos/hero/break_free/posters/break_free_1280.WQ46YIY6.avif';
var breakFreePng1280 = 'assets/videos/hero/break_free/posters/break_free_1280.V35JZ763.png';
var breakFreeWebp1280 = 'assets/videos/hero/break_free/posters/break_free_1280.DWFB27YG.webp';
var breakFreeAvif1920 = 'assets/videos/hero/break_free/posters/break_free_1920.FZMHEY63.avif';
var breakFreePng1920 = 'assets/videos/hero/break_free/posters/break_free_1920.WBZ3NBFU.png';
var breakFreeWebp1920 = 'assets/videos/hero/break_free/posters/break_free_1920.3GCUVDAP.webp';
var breakFreeAvif2560 = 'assets/videos/hero/break_free/posters/break_free_2560.LPKYWG2I.avif';
var breakFreePng2560 = 'assets/videos/hero/break_free/posters/break_free_2560.3L44PPVD.png';
var breakFreeWebp2560 = 'assets/videos/hero/break_free/posters/break_free_2560.JM4UI52R.webp';
var breakFreeAvif3840 = 'assets/videos/hero/break_free/posters/break_free_3840.PEEQKWAO.avif';
var breakFreePng3840 = 'assets/videos/hero/break_free/posters/break_free_3840.IHJWGFLI.png';
var breakFreeWebp3840 = 'assets/videos/hero/break_free/posters/break_free_3840.VNNHBF4O.webp';
var breakFreeAvif426 = 'assets/videos/hero/break_free/posters/break_free_426.A3KD2OWB.avif';
var breakFreePng426 = 'assets/videos/hero/break_free/posters/break_free_426.GHKJUUS4.png';
var breakFreeWebp426 = 'assets/videos/hero/break_free/posters/break_free_426.SJ76L2AA.webp';
var breakFreeAvif640 = 'assets/videos/hero/break_free/posters/break_free_640.BZMJNMOS.avif';
var breakFreePng640 = 'assets/videos/hero/break_free/posters/break_free_640.7RM4H6NV.png';
var breakFreeWebp640 = 'assets/videos/hero/break_free/posters/break_free_640.6W5EO4MO.webp';
var breakFreeAvif854 = 'assets/videos/hero/break_free/posters/break_free_854.T7N3S3Q2.avif';
var breakFreePng854 = 'assets/videos/hero/break_free/posters/break_free_854.44NAZCIU.png';
var breakFreeWebp854 = 'assets/videos/hero/break_free/posters/break_free_854.SBEKVABD.webp';
var tokyoShuffleAvif1280 =
  'assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_1280.ZLQHM4VG.avif';
var tokyoShufflePng1280 =
  'assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_1280.OYFSUR65.png';
var tokyoShuffleWebp1280 =
  'assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_1280.EEEVR5AA.webp';
var tokyoShuffleAvif1920 =
  'assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_1920.AGNLWXP5.avif';
var tokyoShufflePng1920 =
  'assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_1920.C5GVT334.png';
var tokyoShuffleWebp1920 =
  'assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_1920.A6XB2PAB.webp';
var tokyoShuffleAvif2560 =
  'assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_2560.BOHJXWWH.avif';
var tokyoShufflePng2560 =
  'assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_2560.B7WTBGPV.png';
var tokyoShuffleWebp2560 =
  'assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_2560.K223UIDG.webp';
var tokyoShuffleAvif3840 =
  'assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_3840.6YTTSAWO.avif';
var tokyoShufflePng3840 =
  'assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_3840.KWJHCM5F.png';
var tokyoShuffleWebp3840 =
  'assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_3840.WPHZ3N2S.webp';
var tokyoShuffleAvif426 =
  'assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_426.JEFZFOJK.avif';
var tokyoShufflePng426 = 'assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_426.36AKL4KN.png';
var tokyoShuffleWebp426 =
  'assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_426.VQWFGCSR.webp';
var tokyoShuffleAvif640 =
  'assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_640.KF7GISVX.avif';
var tokyoShufflePng640 = 'assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_640.Q7RF6Q5K.png';
var tokyoShuffleWebp640 =
  'assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_640.3AQWY55B.webp';
var tokyoShuffleAvif854 =
  'assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_854.R5Q7G7ZO.avif';
var tokyoShufflePng854 = 'assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_854.AHAG6T2U.png';
var tokyoShuffleWebp854 =
  'assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_854.OH5ALQCN.webp';
var tokyoShuffleAv11280 = 'assets/videos/hero/tokyo_shuffle/tokyo_shuffle_av1_1280.HOVH2ZR4.webm';
var tokyoShuffleAv11920 = 'assets/videos/hero/tokyo_shuffle/tokyo_shuffle_av1_1920.S6SW5VQE.webm';
var tokyoShuffleAv12560 = 'assets/videos/hero/tokyo_shuffle/tokyo_shuffle_av1_2560.SZHPXVXL.webm';
var tokyoShuffleAv13840 = 'assets/videos/hero/tokyo_shuffle/tokyo_shuffle_av1_3840.HZGTVYOV.webm';
var tokyoShuffleAv1426 = 'assets/videos/hero/tokyo_shuffle/tokyo_shuffle_av1_426.GAECFWY2.webm';
var tokyoShuffleAv1640 = 'assets/videos/hero/tokyo_shuffle/tokyo_shuffle_av1_640.ORB4EHAI.webm';
var tokyoShuffleAv1854 = 'assets/videos/hero/tokyo_shuffle/tokyo_shuffle_av1_854.WQWUD7KT.webm';
var tokyoShuffleH2641280 = 'assets/videos/hero/tokyo_shuffle/tokyo_shuffle_h264_1280.UEW2GZ5G.mp4';
var tokyoShuffleH2641920 = 'assets/videos/hero/tokyo_shuffle/tokyo_shuffle_h264_1920.NUVJGLAE.mp4';
var tokyoShuffleH2642560 = 'assets/videos/hero/tokyo_shuffle/tokyo_shuffle_h264_2560.QCYDSUKP.mp4';
var tokyoShuffleH2643840 = 'assets/videos/hero/tokyo_shuffle/tokyo_shuffle_h264_3840.O6N3I74H.mp4';
var tokyoShuffleH264426 = 'assets/videos/hero/tokyo_shuffle/tokyo_shuffle_h264_426.FHTJAQ7F.mp4';
var tokyoShuffleH264640 = 'assets/videos/hero/tokyo_shuffle/tokyo_shuffle_h264_640.XJR44NAQ.mp4';
var tokyoShuffleH264854 = 'assets/videos/hero/tokyo_shuffle/tokyo_shuffle_h264_854.OGGAC5Y3.mp4';
var tokyoShuffleVp91280 = 'assets/videos/hero/tokyo_shuffle/tokyo_shuffle_vp9_1280.DGEJFLP3.webm';
var tokyoShuffleVp91920 = 'assets/videos/hero/tokyo_shuffle/tokyo_shuffle_vp9_1920.HOBXIKVP.webm';
var tokyoShuffleVp92560 = 'assets/videos/hero/tokyo_shuffle/tokyo_shuffle_vp9_2560.ZJ2Y4UKP.webm';
var tokyoShuffleVp93840 = 'assets/videos/hero/tokyo_shuffle/tokyo_shuffle_vp9_3840.2AQHLS5D.webm';
var tokyoShuffleVp9426 = 'assets/videos/hero/tokyo_shuffle/tokyo_shuffle_vp9_426.VL5II3IX.webm';
var tokyoShuffleVp9640 = 'assets/videos/hero/tokyo_shuffle/tokyo_shuffle_vp9_640.Z6MD7AMQ.webm';
var tokyoShuffleVp9854 = 'assets/videos/hero/tokyo_shuffle/tokyo_shuffle_vp9_854.VFGFZLZS.webm';
var origin = window.location.origin || import.meta.url.replace('docs', '');
var toUrl = (pathname) => {
  pathname = pathname.replace('~', '').replace('..', 'assets');
  return new URL(pathname, origin);
};
var plainLogo = toUrl(svgLogo);
var rawHeroVideos = [
  {
    variants: {
      av1: {
    ...
    poster: {
      avif: {
        ...
        srcset: ''
          .concat(tokyoShuffleAvif3840, ' 3840w, ')
          .concat(tokyoShuffleAvif2560, ' 2560w, ')
          .concat(tokyoShuffleAvif1920, ' 1920w, ')
          .concat(tokyoShuffleAvif1280, ' 1280w, ')
          .concat(tokyoShuffleAvif854, ' 854w, ')
          .concat(tokyoShuffleAvif640, ' 640w, ')
          .concat(tokyoShuffleAvif426, ' 426w'),
      },
      webp: {
        widths: {
        ...
        srcset: ''
          .concat(tokyoShuffleWebp3840, ' 3840w, ')
          .concat(tokyoShuffleWebp2560, ' 2560w, ')
          .concat(tokyoShuffleWebp1920, ' 1920w, ')
          .concat(tokyoShuffleWebp1280, ' 1280w, ')
          .concat(tokyoShuffleWebp854, ' 854w, ')
          .concat(tokyoShuffleWebp640, ' 640w, ')
          .concat(tokyoShuffleWebp426, ' 426w'),
      },
      png: {
        widths: {
        ...
        srcset: ''
          .concat(tokyoShufflePng3840, ' 3840w, ')
          .concat(tokyoShufflePng2560, ' 2560w, ')
          .concat(tokyoShufflePng1920, ' 1920w, ')
          .concat(tokyoShufflePng1280, ' 1280w, ')
          .concat(tokyoShufflePng854, ' 854w, ')
          .concat(tokyoShufflePng640, ' 640w, ')
          .concat(tokyoShufflePng426, ' 426w'),
      },
    },
    parentPath: ''.concat(origin, '/assets/videos/hero/tokyo_shuffle'),
  ...
  },
  {
    ...
    poster: {
      avif: {
        ...
        srcset: ''
          .concat(breakFreeAvif3840, ' 3840w, ')
          .concat(breakFreeAvif2560, ' 2560w, ')
          .concat(breakFreeAvif1920, ' 1920w, ')
          .concat(breakFreeAvif1280, ' 1280w, ')
          .concat(breakFreeAvif854, ' 854w, ')
          .concat(breakFreeAvif640, ' 640w, ')
          .concat(breakFreeAvif426, ' 426w'),
      },
      webp: {
        widths: {
        ...
        srcset: ''
          .concat(breakFreeWebp3840, ' 3840w, ')
          .concat(breakFreeWebp2560, ' 2560w, ')
          .concat(breakFreeWebp1920, ' 1920w, ')
          .concat(breakFreeWebp1280, ' 1280w, ')
          .concat(breakFreeWebp854, ' 854w, ')
          .concat(breakFreeWebp640, ' 640w, ')
          .concat(breakFreeWebp426, ' 426w'),
      },
      png: {
        widths: {
        ...
        srcset: ''
          .concat(breakFreePng3840, ' 3840w, ')
          .concat(breakFreePng2560, ' 2560w, ')
          .concat(breakFreePng1920, ' 1920w, ')
          .concat(breakFreePng1280, ' 1280w, ')
          .concat(breakFreePng854, ' 854w, ')
          .concat(breakFreePng640, ' 640w, ')
          .concat(breakFreePng426, ' 426w'),
      },
    },
    parentPath: ''.concat(origin, '/assets/videos/hero/break_free'),
...
var getH264MediaType = () => 'video/mp4';
...
function getMediaType(type, width) {
  switch (type) {
    ...
    default:
      throw new Error('Unsupported video codec: '.concat(type));
  }
}
...
function getActivityClasses(baseClass) {
  return {
    active: ''.concat(baseClass, '--active'),
    inactive: ''.concat(baseClass, '--inactive'),
  };
}
function filterActiveClass(classList, baseClass) {
...
function toggleActiveClass(el, classBase, makeActive) {
...
var VideoElement = class {
  constructor(heroVideo, properties) {
    this.createEl = (tag, classes) => {
      const el = document.createElement(tag);
      el.classList.add(...classes);
      return el;
    };
    this.video = this.createEl('video', ['hero__video', 'hero__video--active']);
    this.picture = this.createEl('picture', ['hero__poster', 'hero__poster--active']);
    this.backupPicture = this.createEl('picture', ['hero__backup']);
    this.properties = {};
    this.message = '';
    var _a2;
    ...
    this.backupPoster = this.isSamePoster
      ? heroVideo.poster
      : ((_a2 = rawHeroVideos.find((video) => video.baseName === BACKUP_PICTURE)) == null
          ? void 0
          : _a2.poster) || heroVideo.poster;
    ...
    for (const prop of Object.keys(this.properties)) {
      ...
      try {
        logger.debug(
          'Setting property '
            .concat(key, ' to ')
            .concat(this.properties[key], ' for video element'),
        );
    ...
    this.backupPicture = this.isSamePoster
      ? (() => {
          const picture = this.picture.cloneNode(true);
  ...
  constructSources() {
    ...
    for (const [codec, variant] of Object.entries(heroVideo.variants)) {
      if (codec === 'av1' || codec === 'vp9' || codec === 'h264') {
        for (const [width, url2] of Object.entries(variant)) {
          const w = typeof width === 'string' ? parseInt(width, 10) : width;
          const src = document.createElement('source');
          const codecVariant = url2;
          src.src = codecVariant.href;
          src.type = getMediaType(codec, w);
          logger.debug('Adding source: '.concat(src.src, ' with type: ').concat(src.type));
  ...
  getSizes() {
    ...
    for (const width of Object.keys(widths)) {
      ...
      if (width in MAX_WIDTHS) {
        sizes +=
          w !== 3840
            ? '(max-width: '.concat(MAX_WIDTHS[width], 'px) ').concat(width, 'px, ')
            : ''.concat(width, 'px');
      }
    }
  ...
  constructPictureElement(picture = this.picture, poster = this.poster) {
    let srcs = [];
...
var TimelineManager = class {
  ...
  onInit() {
    ...
    logger.debug(
      "Timeline's duration: ",
      (_a2 = this.currentTimeline) == null ? void 0 : _a2.duration(),
    );
  ...
  onEnd() {
    ...
    logger.debug(
      'Timeline ended at time:',
      (_a2 = this.currentTimeline) == null ? void 0 : _a2.time(),
    );
    this.transition();
    logger.debug(
      'Transition complete to timeline: '.concat(this.getTimelineName(this.currentTimeline)),
    );
  }
  add(timeline2) {
    var _a2;
  ...
  video() {
    return this.timelines.find(
      (timeline2) =>
        this.getTimelineName(timeline2) === 'videoTimeline' ||
        timeline2['media'] instanceof HTMLVideoElement,
    );
  }
  text() {
...
var VideoManager = class _VideoManager {
  constructor() {
        ...
        const parent =
          ...
          (() => {
            ...
            requestAnimationFrame(() => {
              var _a2, _b;
              (_b =
                (_a2 = document.getElementsByTagName('section')[0]) == null
                  ? void 0
                  : _a2.getElementsByTagName('div')[2]) == null
                ? void 0
                : _b.append(div2);
            });
            return div2;
    ...
    this.container =
      document.querySelector('.hero.first .media__container') ||
      (() => {
        const div = document.createElement('div');
    ...
    this.ctaContainer =
      document.querySelector('.cta__container') ||
      (() => {
        const div = document.createElement('div');
    ...
    this.video = new VideoElement(gsapWithCSS.utils.random(this.videoStore));
  ...
  initSubscriptions() {
    ...
    const errorHandler$ = defer(() => of(gsapWithCSS.utils.toArray(this.element.children))).pipe(
      map((elementChildren) => elementChildren),
      switchMap((elementChildren) => {
        return merge([
          fromEvent(this.element, 'error', (event) => event),
          fromEvent(elementChildren, 'error', (event) => event),
        ]);
      }),
      mergeAll(),
      filter((ev) => {
    ...
    const atHome$ = state$.pipe(
      ...
      exhaustMap(() => {
        if (!this.initialized) {
          ...
          gsapWithCSS.set([this.container, this.poster], show());
          requestAnimationFrame(() => {
  ...
  exchangePosters(activeEl, inactiveElement) {
  ...
  buildTextTimeline() {
    const textTimeline = gsapWithCSS
      .timeline([
        'textTimeline',
        {
  ...
  setEmphasisAnimations() {
    ...
    const subtleTargets = gsapWithCSS.utils.toArray(subtle);
    const strongTargets = gsapWithCSS.utils.toArray(strong);
    const tl = gsapWithCSS.timeline([
      'emphasisTimeline',
      {
        repeat: -1,
  ...
  mediaTimeline() {
    ...
    const existingOnUpdate = config6 && config6.onUpdate;
    const tl = gsapWithCSS.timeline([
      'videoTimeline',
      {
        ...config6,
  ...
  beginPlay() {
    ...
    if (this.timelineManager.timelines.length && !this.timelineManager.isActive()) {
      ...
      if (
        ((_a2 = this.timelineManager.currentTimeline) == null ? void 0 : _a2['media']) &&
        !((_b = this.timelineManager.currentTimeline) == null ? void 0 : _b.duration())
      ) {
  ...
  setVideoDimensionVars() {
    if (!this.onFallback && this.canPlay && this.element.readyState >= 3) {
      ...
      if (height && width) {
        ...
        setCssVariable('--video-height', ''.concat(height, 'px'));
        setCssVariable('--video-width', ''.concat(width, 'px'));
        setCssVariable('--video-left', ''.concat(absoluteLeft, 'px'));
        logger.debug(
          'Set video dimensions: '.concat(width, 'x').concat(height, ', ').concat(absoluteLeft),
        );
      }
    }
  }
  ...
  initiateFallback() {
    ...
    newTl
      .add(['fallback', gsapWithCSS.set(this.element, { ...hide(), duration: 0.5 })], 0)
      .call(() => {
        requestAnimationFrame(() => {
          toggleActiveClass(this.element, 'hero__video', false);
          if (elementInDom(this.element)) {
      ...
      .set(gsapWithCSS.utils.toArray('*', this.ctaContainer), show())
  ...
  debugDump() {
    ...
    logger.debug('Debugging VideoManager timelines:', {
      ...
      currentTimescale:
        (_a2 = this.timelineManager.currentTimeline) == null ? void 0 : _a2.timeScale(),
      ...
      textTimeScale: textTl == null ? void 0 : textTl.timeScale(),
...
var TabManager = class {
    ...
    return inputs
      .map((input) => {
        ...
        const elements = {
          ...
          iconSVG:
            (_a2 = document.querySelector(''.concat(this.selectors.iconPrefix).concat(id))) == null
              ? void 0
              : _a2.querySelector('svg'),
  ...
  setAria() {
    for (const tab of this.tabs) {
      ...
      contentElement.setAttribute(
        'aria-labelledby',
        ''.concat(label.id || labelAnchor.id, ' ').concat(iconAnchor.id),
      );
  ...
  setupInteractions() {
    ...
    return merge(...interactionStreams, ...iconClicks, ...childClicks, ...selections).pipe(
      filter((event) => typeof event === 'object' && 'id' in event && 'event' in event),
      debounceTime(30),
      tap((event) => {
...
function initLicenseFeature() {
  let tabManager = null;
  return document$3.pipe(
    tap(() => {
      var _a2;
      (_a2 = tabManager == null ? void 0 : tabManager.cleanup) == null
        ? void 0
        : _a2.call(tabManager);
...
var imgLogo = document.querySelector('img.simple_logo');
if (
  imgLogo &&
...
var license$ = navigationEvents$.pipe(
  filter(isLicense),
...
var pageConfigs = [
  {
    matcher: isHome,
    location: 'home',
    observables: [nav$, color$, observer$4, videoManager$],
  },
  {
    matcher: isLicense,
    location: 'licenses',
...
var pageSubscription$ = navigationEvents$.pipe(
  map((url2) => {
    const matchingConfigs = pageConfigs.filter((config6) => config6.matcher(url2));
    if (matchingConfigs.length === 0) {
      return null;
    }
    matchingConfigs.forEach((config6) => logger.info('Navigated to '.concat(config6.location)));
    return matchingConfigs;
  }),
  filter((configs) => configs !== null),
  switchMap((configs) => {
    const allObservables = configs.flatMap((config6) =>
      config6.observables.map((obs) =>
        obs.pipe(
          tap(() => logger.info('Running observables for '.concat(config6.location))),
          catchError((error) => {
            logger.error('Error in '.concat(config6.location, ' observables:'), error);
            return EMPTY;
          }),
        ),
      ),
...
gsap/ScrollTrigger.js:
  ...
  *)
*/
//# sourceMappingURL=index.CCIGGCML.js.map
