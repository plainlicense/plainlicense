...
function mountSidebar(el, { header$: header$3, ...options }) {
  ...
  return defer(() => {
    ...
    from(getElements("label[tabindex]", el)).pipe(
      ...
      const nav = getElement('[aria-labelledby="'.concat(label.id, '"]'));
      nav.setAttribute("aria-expanded", "".concat(input.checked));
    });
...
var cacheWorkerUrl = "cacheWorker.HY2NZKN2.js";
...
function wordsToLetterDivs(el) {
  ...
  gsapWithCSS.set(fragmentDivs, { display: "block", autoAlpha: 0 });
  return docFragment;
}
...
gsapWithCSS.registerEffect({
  ...
  effect: (targets, config6) => {
    ...
    const containerTarget = (targets instanceof Array ? targets[0] : targets) || document.querySelector(".hero__container");
    if (!containerTarget) {
    ...
    if (config6.message) {
      ...
      messageTimeline.add(gsapWithCSS.set(containerTarget, { contentVisibility: "visible" })).add([
        "randomEntrance",
...
function toggleActiveClass(el, classBase, makeActive) {
  ...
  const alreadyPresent = el.classList.contains(makeActive ? active : inactive) && !el.classList.contains(makeActive ? inactive : active);
  if (alreadyPresent) {
    return;
  }
...
var VideoManager = class _VideoManager {
  constructor() {
    this.store = HeroStore.getInstance();
    this.container = document.querySelector(".hero.first .hero__container") || (() => {
    ...
    this.logo = (() => {
      ...
      img.width = gsapWithCSS.utils.clamp(64, 400, viewportWidth * 0.1);
  ...
  init_subscriptions() {
    ...
    const textAnimator$ = fromEvent(this.element, "timeupdate").pipe(filter((ev) => {
      return ev.target instanceof HTMLMediaElement && ev.target.duration > 0 && ev.target.currentTime >= this.titleStart;
    }), map(() => {
      return true;
  ...
  animateText(overrideVars = {}) {
    ...
    }).set(this.container, { autoAlpha: 1 }).to(this.ctaContainer, { autoAlpha: 1, duration: 0.5 })["animateMessage"](this.container, {
...
var HeroStore = class _HeroStore {
  ...
  initSubscriptions() {
    ...
    const header$3 = watchHeader2(getComponentElement("header"), { viewport$: viewport$4 }).pipe(tap((header) => {
...
gsap/ScrollToPlugin.js:
  ...
  *)
*/
//# sourceMappingURL=index.KOV27CWI.js.map
