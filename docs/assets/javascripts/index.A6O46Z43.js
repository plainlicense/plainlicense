...
var elementIsVisible = (el, checkParent = false) => {
  var _a2, _b;
...
var cacheWorkerUrl = "cacheWorker.4PJF5622.js";
...
function toggleActiveClass(el, classBase, makeActive) {
  ...
  Array.from(el.classList).filter((c) => makeActive ? c.endsWith("inactive") : c.endsWith("active")).filter((c) => c && c.length).forEach((c) => el.classList.remove(c));
}
...
var VideoManager = class _VideoManager {
  constructor() {
    ...
    this.canPlay = false;
    this.videoTimeline = gsapWithCSS.timeline({
  ...
  get videoDuration() {
    var _a2, _b;
    return (_b = (_a2 = this.element) == null ? void 0 : _a2.duration) != null ? _b : VIDEO_END_BUFFER;
  }
  get titleStart() {
  ...
  initSubscriptions() {
    ...
    const statusSetter$ = state$.pipe(map(({ canPlay, prefersReducedMotion, currentSection }) => {
      return { canPlay, prefersReducedMotion, currentSection };
    }), tap(({ prefersReducedMotion }) => {
      var _a2;
      if (prefersReducedMotion) {
        this.masterTimeline.pause();
        (_a2 = this.element) == null ? void 0 : _a2.pause();
        this.canPlay = false;
        this.status = "on_fallback";
        this.initiateFallback();
      }
    }), distinctUntilChanged((prev, cur) => prev.canPlay === cur.canPlay && prev.currentSection === cur.currentSection), debounceTime(100), tap(({ prefersReducedMotion, currentSection, canPlay }) => {
      if (prefersReducedMotion) {
        return;
      }
      ...
      if (canPlay) {
        if (!videoInDom || !posterInDom || !this.element.duration) {
          this.status = "not_initialized";
        } else if (posterInDom && videoInDom && (!this.element.duration || this.element.readyState <= 3)) {
          this.status = "loading";
        } else if (videoInDom && this.element.readyState === 4 && !this.isPlaying()) {
          if (currentSection >= 1 /* Impact */) {
            this.status = "paused";
          } else {
            this.status = "loaded";
          }
          ...
          this.status = "playing";
        } else if (this.element.readyState === 4 && this.masterTimeline.totalDuration() > 0 && this.masterTimeline.isActive() && this.masterTimeline.currentLabel() === "textTimeline") {
          this.status = "on_textAnimation";
        }
      } else if (this.status === "on_fallback") {
        return;
      } else if (currentSection >= 1 /* Impact */ && this.status !== "not_initialized") {
        this.status = "paused";
    ...
    const play$ = canplaythrough$.pipe(filter(() => this.canPlay === true && this.element.readyState === 4 && !this.isPlaying()), tap(() => {
    ...
    const motionSub$ = this.store.state$.pipe(distinctUntilKeyChanged("prefersReducedMotion"), filter(({ prefersReducedMotion }) => prefersReducedMotion), tap(() => {
      var _a2;
      this.masterTimeline.pause();
      (_a2 = this.element) == null ? void 0 : _a2.pause();
      this.canPlay = false;
    ...
    ]).pipe(takeUntil(canplaythrough$), switchMap(() => {
    ...
    const varSet$ = fromEvent(this.element, "loadedmetadata").pipe(filter((ev) => ev && ev.target instanceof HTMLMediaElement), map(({ target }) => target), take(1), tap((target) => {
    ...
    const resize$ = this.store.state$.pipe(skipUntil(varSet$), map(({ viewport: { offset } }) => offset), distinctUntilKeyChanged("y"), debounceTime(500), distinctUntilChanged((previous, current) => {
    ...
    const playing$ = fromEvent(this.element, "playing").pipe(filter((ev) => {
      return ev.target instanceof HTMLMediaElement;
    }), distinctUntilChanged(), filter(() => this.canPlay === true && this.status !== "playing" && this.status !== "on_fallback"), tap(() => {
      if (!this.isPlaying()) {
        this.foulPlay();
      }
    ...
    }), filter(() => this.canPlay === true && this.status !== "playing" && this.status !== "on_fallback" && this.element.readyState === 4), switchMap(() => play$));
  ...
  animateText(overrideVars = {}) {
    return gsapWithCSS.timeline().set(".text-animation__container", { autoAlpha: 1 })["animateMessage"](".text-animation__container", {
  ...
  syncToVideo() {
    logger.debug("Syncing video to ".concat(this.timeLeft(), " seconds"));
    this.videoTimeline.isActive() ? this.videoTimeline.duration(this.timeLeft() - (VIDEO_END_BUFFER - 0.5)) : this.videoTimeline.duration(this.videoDuration);
  }
  // sets initial timeline properties
  buildVideoTimeline() {
    logger.debug("Building the video timeline");
    this.videoTimeline.add([
      "startVideo",
      () => {
    ...
    ], "<").set(this.container, { autoAlpha: 1 }).set(this.element, { autoAlpha: 1 }).set(this.ctaContainer, { autoAlpha: 1 }).add(
      // we make sure the timeline syncs with the video
      gsapWithCSS.delayedCall(2, () => {
        if (this.isPlaying() && this.videoTimeline.totalDuration() > this.timeLeft() - VIDEO_END_BUFFER) {
          this.syncToVideo();
        } else if (!this.isPlaying()) {
          this.videoTimeline.restart().pause();
          this.foulPlay();
  ...
  buildTextTimeline() {
    ...
    this.textAnimationTimeline.add(gsapWithCSS.to(ctaHeaders, {
      autoAlpha: 0,
      duration: 0.5
    }), "<").set(this.element, { autoAlpha: 0 }).add(this.animateText(), "<=+0.5").add(gsapWithCSS.to(ctaHeaders, {
      autoAlpha: 1,
      duration: 0.5
    }), ">").add(() => {
  ...
  loadBackup() {
    if (!elementInDom(this.backupPicture)) {
      requestAnimationFrame(() => {
        var _a2;
        this.container.append(this.backupPicture);
        (_a2 = this.backupPicture.querySelector("img")) == null ? void 0 : _a2.classList.add("hero__poster--image");
  ...
  isPlaying() {
    return this.element && this.element.duration > 0 && this.element.currentTime > 0 && !this.element.paused && !this.element.ended;
  }
  timeLeft() {
  ...
  play() {
    if (!this.masterTimeline.isActive() && this.status !== "on_fallback" && this.status !== "on_textAnimation") {
      this.transitionToVideo();
...
gsap/ScrollToPlugin.js:
  ...
  *)
*/
//# sourceMappingURL=index.A6O46Z43.js.map
