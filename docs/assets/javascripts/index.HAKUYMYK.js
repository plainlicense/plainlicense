...
var init_gsap_core = __esm({
  "node_modules/gsap/gsap-core.js"() {
    ...
    _createPlugin = function _createPlugin2(config6) {
      if (!config6) return;
...
var init_CSSPlugin = __esm({
  "node_modules/gsap/CSSPlugin.js"() {
    ...
    _parseTransform = function _parseTransform2(target, uncache) {
      ...
      var style = target.style, invertedScaleX = cache.scaleX < 0, px = "px", deg = "deg", cs = getComputedStyle(target), origin2 = _getComputedProperty(target, _transformOriginProp) || "0", x, y, z, scaleX, scaleY, rotation, rotationX, rotationY, skewX, skewY, perspective, xOrigin, yOrigin, matrix, angle, cos, sin, a, b, c, d, a12, a22, t1, t2, t3, a13, a23, a33, a42, a43, a32;
...
var EXCLUDED_TAGS, BACKUP_PICTURE, FADE_IN_CONFIG, OBSERVER_CONFIG, VIDEO_MANAGER_ELEMENTS, MAX_WIDTHS, SUBTLE_EMPHASIS_CONFIG, STRONG_EMPHASIS_CONFIG;
var init_config = __esm({
  "src/assets/javascripts/config/config.ts"() {
    "use strict";
    init_gsap();
    EXCLUDED_TAGS = ["STYLE", "SCRIPT", "NOSCRIPT"];
    BACKUP_PICTURE = "break_free";
    FADE_IN_CONFIG = {
    ...
    VIDEO_MANAGER_ELEMENTS = ".hero__video, .hero__poster, .hero__poster--image, .hero__backup, noscript, style, script";
    MAX_WIDTHS = {
      426: "426",
      640: "640",
      854: "854",
...
// src/assets/javascripts/config/types.ts
...
// src/assets/javascripts/config/index.ts
...
function elementsNotVisible(parent = document.body) {
  const children = gsapWithCSS.utils.toArray("*", parent);
  return children.filter((el) => isValidElement(el, parent) && !elementIsVisible(el)).filter(Boolean);
}
function generateNonVisibleElementReport(parent = document.body) {
  ...
  return elements.map((el) => {
    ...
    const report = {
      ...
      parentHidden: (parentComputedStyle.display === "none" || parentComputedStyle.visibility === "hidden") && computedStyle.visibility === "inherit",
    ...
    if (Object.entries(report).some(([_, value]) => value)) {
      report.reason = Object.entries(report).map(([key, value]) => {
...
var LICENSE_HASHES, isProd, isDev, isOnSite, isHome, isMainSiteLicensePage, isEmbeddedLicensePage, isLicense, isLicenseHash, isHelpingIndex, isValidEvent, elementIsVisible;
var init_conditionChecks = __esm({
  "src/assets/javascripts/utils/conditionChecks.ts"() {
    "use strict";
    init_gsap();
    init_config2();
    LICENSE_HASHES = ["#reader", "#html", "#markdown", "#plaintext", "#changelog", "#official"];
    isProd = (url2) => {
    ...
    elementIsVisible = (el, checkParent = false) => {
      var _a2, _b;
      ...
      const isNotHidden = el.style.display !== "none" && el.style.visibility !== "hidden" && el.style.opacity !== "0";
      const parentNotHidden = ((_a2 = el.parentElement) == null ? void 0 : _a2.style.contentVisibility) !== "hidden" && ((_b = el.parentElement) == null ? void 0 : _b.style.visibility) !== "hidden";
...
var isDevelopment, url, logger;
var init_log = __esm({
  "src/assets/javascripts/utils/log.ts"() {
    "use strict";
    init_conditionChecks();
    isDevelopment = false;
...
function parsePath(path) {
  ...
  try {
    ...
    urlObj = {
      ...
      search: url2.search.length ? Object.fromEntries(new URLSearchParams(url2.search).entries()) || "" : url2.search || "",
...
var createScript, piSmasher, stringify;
var init_helpers = __esm({
  "src/assets/javascripts/utils/helpers.ts"() {
    "use strict";
    init_log();
    createScript = (src, async2 = true, defer2 = true, ignoreDnt = false) => {
    ...
    piSmasher = getCircularReplacer();
    stringify = (obj) => JSON.stringify(obj, piSmasher, 2);
  }
});
...
function errorContext(cb) {
  if (config3.useDeprecatedSynchronousErrorHandling) {
    ...
    cb();
    if (isRoot) {
      var _a2 = context3, errorThrown = _a2.errorThrown, error = _a2.error;
      context3 = null;
      if (errorThrown) {
        throw error;
      }
...
function getPromiseCtor(promiseCtor) {
  var _a2;
  return (_a2 = promiseCtor !== null && promiseCtor !== void 0 ? promiseCtor : config3.Promise) !== null && _a2 !== void 0 ? _a2 : Promise;
}
...
var init_Observable = __esm({
  "node_modules/rxjs/dist/esm5/internal/Observable.js"() {
    ...
    init_errorContext();
    Observable = function() {
      function Observable63(subscribe) {
        if (subscribe) {
          this._subscribe = subscribe;
        }
      }
      Observable63.prototype.lift = function(operator) {
        var observable2 = new Observable63();
        observable2.source = this;
        observable2.operator = operator;
        return observable2;
      };
      Observable63.prototype.subscribe = function(observerOrNext, error, complete) {
        var _this = this;
      ...
      Observable63.prototype._trySubscribe = function(sink) {
        try {
      ...
      Observable63.prototype.forEach = function(next, promiseCtor) {
        var _this = this;
      ...
      Observable63.prototype._subscribe = function(subscriber) {
        var _a2;
        return (_a2 = this.source) === null || _a2 === void 0 ? void 0 : _a2.subscribe(subscriber);
      };
      Observable63.prototype[observable] = function() {
        return this;
      };
      Observable63.prototype.pipe = function() {
        var operations = [];
        for (var _i2 = 0; _i2 < arguments.length; _i2++) {
      ...
      Observable63.prototype.toPromise = function(promiseCtor) {
        var _this = this;
        promiseCtor = getPromiseCtor(promiseCtor);
        return new promiseCtor(function(resolve3, reject) {
          var value;
      ...
      Observable63.create = function(subscribe) {
        return new Observable63(subscribe);
      };
      return Observable63;
    }();
  }
...
function throttle(durationSelector, config6) {
  return operate(function(source, subscriber) {
    var _a2 = config6 !== null && config6 !== void 0 ? config6 : {}, _b = _a2.leading, leading = _b === void 0 ? true : _b, _c = _a2.trailing, trailing = _c === void 0 ? false : _c;
    var hasValue = false;
    var sendValue = null;
    var throttled = null;
    var isComplete = false;
...
function getElement(selector3, node = document) {
  ...
  if (typeof el === "undefined")
    throw new ReferenceError(
      'Missing element: expected "'.concat(selector3, '" to be present')
    );
  return el;
}
...
// external/mkdocs-material/src/templates/assets/javascripts/utilities/index.ts
...
var init__2 = __esm({
  "external/mkdocs-material/src/templates/assets/javascripts/browser/element/size/_/index.ts"() {
    ...
    init_script();
    entry$ = new Subject();
    observer$2 = defer(() => typeof ResizeObserver === "undefined" ? watchScript("https://unpkg.com/resize-observer-polyfill") : of(void 0)).pipe(
...
// external/mkdocs-material/src/templates/assets/javascripts/browser/element/size/index.ts
var init_size = __esm({
  "external/mkdocs-material/src/templates/assets/javascripts/browser/element/size/index.ts"() {
    "use strict";
    init__2();
...
// external/mkdocs-material/src/templates/assets/javascripts/browser/element/offset/index.ts
...
// external/mkdocs-material/src/templates/assets/javascripts/browser/element/index.ts
...
// external/mkdocs-material/src/templates/assets/javascripts/browser/location/index.ts
...
// external/mkdocs-material/src/templates/assets/javascripts/browser/viewport/index.ts
...
// external/mkdocs-material/src/templates/assets/javascripts/browser/index.ts
...
var init__6 = __esm({
  "external/mkdocs-material/src/templates/assets/javascripts/_/index.ts"() {
    "use strict";
    init_browser();
    script = getElement("#__config");
    config4 = JSON.parse(script.textContent);
...
function renderSearchDocument(document2, flag) {
  ...
  if (feature("search.highlight"))
    url2.searchParams.set(
      "h",
      Object.entries(document2.terms).filter(([, match]) => match).reduce((highlight2, [value]) => "".concat(highlight2, " ").concat(value).trim(), "")
    );
  const { tags } = configuration();
  return /* @__PURE__ */ h("a", { href: "".concat(url2), class: "md-search-result__link", tabIndex: -1 }, /* @__PURE__ */ h(
    "article",
    {
    ...
    document2.tags && /* @__PURE__ */ h("nav", { class: "md-tags" }, document2.tags.map((tag) => {
      const type = tags ? tag in tags ? "md-tag-icon md-tag--".concat(tags[tag]) : "md-tag-icon" : "";
...
function renderSourceFacts(facts) {
  return /* @__PURE__ */ h("ul", { class: "md-source__facts" }, Object.entries(facts).map(([key, value]) => /* @__PURE__ */ h("li", { class: "md-source__fact md-source__fact--".concat(key) }, typeof value === "number" ? round(value) : value)));
}
...
function renderVersion(version) {
  ...
  return /* @__PURE__ */ h("li", { class: "md-version__item" }, /* @__PURE__ */ h("a", { href: "".concat(url2), class: "md-version__link" }, version.title, ((_a2 = config6.version) == null ? void 0 : _a2.alias) && version.aliases.length > 0 && /* @__PURE__ */ h("span", { class: "md-version__alias" }, version.aliases[0])));
}
function renderVersionSelector(versions, active) {
  ...
  return /* @__PURE__ */ h("div", { class: "md-version" }, /* @__PURE__ */ h(
    ...
    active.title,
    ((_a2 = config6.version) == null ? void 0 : _a2.alias) && active.aliases.length > 0 && /* @__PURE__ */ h("span", { class: "md-version__alias" }, active.aliases[0])
...
// external/mkdocs-material/src/templates/assets/javascripts/templates/index.ts
...
function mountAnnotation(el, container, { target$: target$2 }) {
  ...
  return defer(() => {
    ...
    push$.pipe(
      ...
      next(origin2) {
        if (origin2)
          el.style.setProperty("--md-tooltip-0", "".concat(-origin2, "px"));
        else
...
// external/mkdocs-material/src/templates/assets/javascripts/components/content/annotation/index.ts
var init_annotation2 = __esm({
  "external/mkdocs-material/src/templates/assets/javascripts/components/content/annotation/index.ts"() {
    "use strict";
    init__8();
    init_block();
...
var import_clipboard, sequence2;
var init__9 = __esm({
  "external/mkdocs-material/src/templates/assets/javascripts/components/content/code/_/index.ts"() {
    "use strict";
    import_clipboard = __toESM(require_clipboard());
    init_esm5();
    init__6();
    init_browser();
    init_tooltip2();
...
// external/mkdocs-material/src/templates/assets/javascripts/components/content/code/index.ts
var init_code = __esm({
  "external/mkdocs-material/src/templates/assets/javascripts/components/content/code/index.ts"() {
    "use strict";
    init__9();
...
var mermaid$, sequence3;
var init_mermaid2 = __esm({
  "external/mkdocs-material/src/templates/assets/javascripts/components/content/mermaid/index.ts"() {
    "use strict";
    init_esm5();
    init_browser();
...
function watchContentTabs(inputs) {
  ...
  return merge(...inputs.map(
    (input) => fromEvent(input, "change").pipe(
      map(() => getElement('label[for="'.concat(input.id, '"]')))
    )
  )).pipe(
...
// external/mkdocs-material/src/templates/assets/javascripts/components/content/index.ts
...
function mountTooltip(el) {
  ...
  return defer(() => {
    ...
    push$.pipe(
      ...
      next(origin2) {
        if (origin2)
          tooltip.style.setProperty("--md-tooltip-0", "".concat(-origin2, "px"));
        else
...
// external/mkdocs-material/src/templates/assets/javascripts/components/header/index.ts
...
var import_clipboard2;
var init_clipboard2 = __esm({
  "external/mkdocs-material/src/templates/assets/javascripts/integrations/clipboard/index.ts"() {
    "use strict";
    import_clipboard2 = __toESM(require_clipboard());
    init_esm5();
    init__6();
    init_browser();
  }
...
// external/mkdocs-material/src/templates/assets/javascripts/integrations/search/config/index.ts
...
// external/mkdocs-material/src/templates/assets/javascripts/integrations/search/internal/_/index.ts
var init__12 = __esm({
  "external/mkdocs-material/src/templates/assets/javascripts/integrations/search/internal/_/index.ts"() {
    "use strict";
  }
...
// external/mkdocs-material/src/templates/assets/javascripts/integrations/search/internal/extract/index.ts
...
// external/mkdocs-material/src/templates/assets/javascripts/integrations/search/internal/highlight/index.ts
...
// external/mkdocs-material/src/templates/assets/javascripts/integrations/search/internal/tokenize/index.ts
...
// external/mkdocs-material/src/templates/assets/javascripts/integrations/search/internal/index.ts
...
// external/mkdocs-material/src/templates/assets/javascripts/integrations/search/query/transform/index.ts
...
// external/mkdocs-material/src/templates/assets/javascripts/integrations/search/query/_/index.ts
var init__13 = __esm({
  "external/mkdocs-material/src/templates/assets/javascripts/integrations/search/query/_/index.ts"() {
    "use strict";
    init_internal();
...
// external/mkdocs-material/src/templates/assets/javascripts/integrations/search/query/segment/index.ts
...
// external/mkdocs-material/src/templates/assets/javascripts/integrations/search/query/index.ts
...
// external/mkdocs-material/src/templates/assets/javascripts/integrations/search/_/index.ts
var init__14 = __esm({
  "external/mkdocs-material/src/templates/assets/javascripts/integrations/search/_/index.ts"() {
    "use strict";
    init_config4();
...
function setupSearchHighlighter(config6) {
  ...
  return (query) => {
    ...
    const match = new RegExp("(^|".concat(config6.separator, "|)(").concat(query.replace(/[|\\{}()[\]^$+*?.-]/g, "\\$&").replace(separator, "|"), ")"), "img");
...
// external/mkdocs-material/src/templates/assets/javascripts/integrations/search/worker/index.ts
...
// external/mkdocs-material/src/templates/assets/javascripts/integrations/search/index.ts
          ...
          return fetchSitemap(selectedVersionBaseURL).pipe(
            map(
              (sitemap) => {
                ...
                })) != null ? _a2 : selectedVersionBaseURL;
              }
            )
          );
        })
...
// external/mkdocs-material/src/templates/assets/javascripts/integrations/index.ts
...
// external/mkdocs-material/src/templates/assets/javascripts/components/search/index.ts
...
// external/mkdocs-material/src/templates/assets/javascripts/components/source/facts/index.ts
...
// external/mkdocs-material/src/templates/assets/javascripts/components/source/index.ts
...
// external/mkdocs-material/src/templates/assets/javascripts/components/index.ts
...
// external/mkdocs-material/src/templates/assets/javascripts/patches/index.ts
...
function fetchSearchIndex() {
  if (location.protocol === "file:") {
    return watchScript(
      "".concat(new URL("search/search_index.js", config5.base))
    ).pipe(
...
var import_focus_visible, document$, location$, target$, keyboard$, viewport$2, tablet$, screen$, print$, config5, index$, alert$, progress$, _a, header$, main$, control$, content$, component$;
var init_bundle = __esm({
  "external/mkdocs-material/src/templates/assets/javascripts/bundle.ts"() {
    "use strict";
    import_focus_visible = __toESM(require_focus_visible());
    init_esm5();
    init__6();
    init_browser();
    init_components();
    ...
    target$ = watchLocationTarget(location$);
    keyboard$ = watchKeyboard();
    viewport$2 = watchViewport();
    tablet$ = watchMedia("(min-width: 960px)");
    screen$ = watchMedia("(min-width: 1220px)");
    print$ = watchPrint();
    config5 = configuration();
    index$ = document.forms.namedItem("search") ? fetchSearchIndex() : NEVER;
    alert$ = new Subject();
    ...
    header$ = watchHeader(getComponentElement("header"), { viewport$: viewport$2 });
    main$ = document$.pipe(
...
function watchHeader2(el, options) {
  return defer(() => combineLatest([watchElementSize(el), isHidden2(options)])).pipe(map(([{ height }, hidden]) => ({
    height,
    hidden
  })), distinctUntilChanged((a, b) => a.height === b.height && a.hidden === b.hidden), shareReplay(1));
}
async function resizeText(header2, offsetX) {
  const headerWidth = () => header2.getBoundingClientRect().width;
  const currentSize = () => parseInt(window.getComputedStyle(header2).fontSize, 10);
  ...
  while (headerWidth() > maxWidth || headerWidth() < maxWidth - 10) {
    ...
    requestAnimationFrame(() => {
      setCssVariable("--hero-h1-size", "".concat(newSize, "px"));
    });
  }
}
async function heroTextSizeWatch() {
  const headers = gsapWithCSS.utils.toArray("section.hero h1").filter((header3) => {
    return header3 instanceof HTMLElement && header3.innerText.length > 0;
  });
  const lineBreak = /<br\s?\/>/;
  const headerTexts = [];
  headers.forEach((header3) => {
    const text = header3.innerHTML;
    if (lineBreak.test(text)) {
      const lines = text.split(lineBreak).map((line) => line.replace(/<[^>]*>/g, "").trim());
      headerTexts.push(lines);
    } else {
      const line = header3.innerText.trim();
      headerTexts.push([line]);
    }
  });
  const longestLine = headerTexts.reduce((acc, lines) => {
    const longest = lines.reduce((longest2, line) => {
      return line.length > longest2.length ? line : longest2 || "";
    }, "");
  ...
  const header2 = headers.find((header3) => {
    return header3.innerText.includes(longestLine);
  });
  if (!header2) {
    return EMPTY;
  }
  return viewport$3.pipe(map(({ offset: { x } }) => x), distinctUntilChanged(), tap((x) => {
    from(resizeText(header2, x)).subscribe();
  }));
}
var import_tablesort, customWindow, location$2, viewport$3, preventDefault, watchMediaQuery, isPageVisible$, prefersReducedMotion$, header$2, mapLocation, navigationEvents$, handleLicenseHash, header, nav, footer;
var init_eventHandlers = __esm({
  "src/assets/javascripts/utils/eventHandlers.ts"() {
    ...
    init_gsap();
    init_esm5();
    import_tablesort = __toESM(require_tablesort());
    init__6();
    init_browser();
    init_components();
    init_utils();
    customWindow = window;
    ({ location$: location$2, viewport$: viewport$3 } = customWindow);
    preventDefault = (ev) => {
      ev.preventDefault();
      return ev;
    };
    ...
    isPageVisible$ = fromEvent(document, "visibilitychange").pipe(map(() => !document.hidden || document.visibilityState === "visible"), debounceTime(200), distinctUntilChanged(), startWith(document.visibilityState === "visible"), shareReplay(1));
    prefersReducedMotion$ = watchMediaQuery("(prefers-reduced-motion: reduce)").pipe(share());
    header$2 = watchHeader2(getComponentElement("header"), { viewport$: viewport$3 });
    mapLocation = (ev) => {
      const eventMap = {
    ...
    navigationEvents$ = "navigation" in customWindow ? (
    ...
    header = getComponentElement("header");
    nav = getComponentElement("tabs");
...
// src/assets/javascripts/state/types.ts
...
// src/assets/javascripts/hero/animations/types.ts
...
function getContentElements(element) {
  ...
  const hasContent = (el) => {
    var _a2, _b, _c;
    return ((_c = (_b = (_a2 = el.textContent) == null ? void 0 : _a2.trim()) == null ? void 0 : _b.length) != null ? _c : 0) > 0 || el.querySelector("img, svg, video, picture") !== null;
  };
...
var require_effects = __commonJS({
  "src/assets/javascripts/hero/animations/effects.ts"(exports) {
    "use strict";
    init_gsap();
    init_config2();
    init_utils();
    ...
    var fade = (targets, config6 = { out: false, direction: 1, fromConfig: {}, toConfig: {} }) => {
      ...
      if (fromConfig && toConfig) {
        fadeVars = getFadeVars(out, parseInt(((_a2 = fromConfig.yPercent) == null ? void 0 : _a2.toString()) || "50", 10) || parseInt(((_b = toConfig == null ? void 0 : toConfig.yPercent) == null ? void 0 : _b.toString()) || "0", 10) || 50, direction || 1);
      } else {
        fadeVars = getFadeVars(out, parseInt(((_c = OBSERVER_CONFIG.fades.fadeInConfig.normal.from.yPercent) == null ? void 0 : _c.toString()) || "50", 10), direction || 1);
      }
    ...
    gsapWithCSS.registerEffect({
      ...
      effect: (targets, config6) => {
        ...
        return gsapWithCSS.timeline({ extendTimeline: true }).set(targets, { zIndex: 1, autoAlpha: 1 }).fromTo([section.outerWrapper, section.innerWrapper], {
          ...
          yPercent: 0,
          zIndex: -500
        }, 0).fromTo(section.bg, { yPercent: 15 * dFactor }, { yPercent: 0, zIndex: -300, opacity: 1 }, 0).add(["fadeIn", fade(fadeInTargets, { ...OBSERVER_CONFIG.fades, out: false, direction })], ">").add(gsapWithCSS.to(section.content, { autoAlpha: 1, zIndex: 1 }), "fadeIn+=0.3").add(revealHeader, ">");
      }
    });
    gsapWithCSS.registerEffect({
      name: "fadeIn",
      ...
      effect: (targets, config6) => {
        ...
        logger.debug("fading in targets: ".concat(logObject(targets, "fadeIn target")));
    ...
    gsapWithCSS.registerEffect({
      ...
      effect: (targets, config6) => {
        const combinedConfig = { ...OBSERVER_CONFIG.fades, ...config6 };
        logger.debug("fadeOut: targets: ".concat(logObject(targets, "fadeOut target")));
        const { direction, fromConfig, toConfig } = combinedConfig;
    ...
    gsapWithCSS.registerEffect({
      ...
      effect: (targets, config6) => {
        logger.debug("emphasize: targets: ".concat(logObject(targets, "emphasize target")));
        if (!targets) {
          return null;
        }
      ...
      effect: (targets, config6) => {
        ...
        if (config6.message) {
          ...
          const messageTimeline = gsapWithCSS.timeline({
            ...
            extendTimeline: true,
            onStart: () => {
              const element = (exports == null ? void 0 : exports.element) || document.querySelector("section.hero.first .hero__video");
              if (!element) {
...
var import_effects, HeroObservation;
var init_observer = __esm({
  "src/assets/javascripts/hero/animations/observer.ts"() {
    "use strict";
    init_gsap();
    ...
    init_utils2();
    init_utils();
    import_effects = __toESM(require_effects());
    gsapWithCSS.registerPlugin(Observer);
    HeroObservation = class _HeroObservation {
      constructor() {
      ...
      setupSubscriptions() {
        const atHome$ = this.store.state$.pipe(filter((state) => state.atHome), distinctUntilKeyChanged("atHome"), map((state) => {
          return state.atHome;
        }), filter((atHome) => atHome));
        const index$2 = this.indexSubject.asObservable().pipe(distinctUntilChanged(), skipUntil(atHome$), distinctUntilChanged());
        const tearDown$ = this.store.state$.pipe(map((state) => state.atHome), distinctUntilChanged(), filter((atHome) => !atHome), debounceTime(5e3));
      ...
      onLoad() {
        ...
        (_a2 = this.header) == null ? void 0 : _a2.forEach((header2) => {
        ...
        this.subscriptions.add(navigationEvents$.pipe(filter((url2) => !isHome(url2))).subscribe(() => {
          var _a3, _b;
          (_a3 = this.header) == null ? void 0 : _a3.forEach((header2) => {
      ...
      setupSections() {
        ...
        this.sections = sectionEls.map((el, index) => {
          ...
          const initialContent = getContentElements(el).filter((element) => element !== outerWrapper && element !== innerWrapper && element !== bg).filter((e) => isValidElement(e, e.parentElement || this.sections[index].bg || this.sections[index].element));
          const content = Array.from(new Set(initialContent));
          logger.debug("Setting up section ".concat(index), { outerWrapper, innerWrapper, bg, content });
          const animation = gsapWithCSS.timeline({
            ...
            paused: true,
            duration: index === 0 /* Landing */ ? () => this.hasTransitioned ? this.config.slides.slideDuration : this.config.slides.slideDuration / 3 : this.config.slides.slideDuration,
            callbackScope: this,
        ...
        this.sections[0].content = this.sections[0].content.filter((content) => this.isValidContentTarget(content));
      ...
      constructTransitionTimeline(direction, index, tl) {
        ...
        logger.debug("Timeline state: currentIndex=".concat(this.currentIndex, ", targetIndex=").concat(index, ", direction=").concat(direction, ", sectionsCount=").concat(this.sectionCount));
      ...
      goToSection(index, direction) {
        ...
        if (!this.sections[clampedIndex].animation) {
          throw new Error("No animation found for section ".concat(clampedIndex, " -- failed to setup transition"));
        }
        logger.debug("Transitioning to section ".concat(clampedIndex, " in direction ").concat(direction));
        this.transitionTl = this.constructTransitionTimeline(direction, clampedIndex, this.sections[clampedIndex].animation);
...
// src/assets/javascripts/hero/animations/index.ts
var import_effects2;
var init_animations = __esm({
  "src/assets/javascripts/hero/animations/index.ts"() {
    ...
    init_types4();
    init_utils2();
    import_effects2 = __toESM(require_effects());
    init_observer();
    init_observer();
    init_types4();
  }
...
// src/assets/javascripts/hero/video/types.ts
...
var breakFreeAv13840 = "assets/videos/hero/break_free/break_free_av1_3840.5V2V4VJR.webm";
var breakFreeAv1426 = "assets/videos/hero/break_free/break_free_av1_426.XMIUTUPS.webm";
var breakFreeAv1640 = "assets/videos/hero/break_free/break_free_av1_640.W4QED7UF.webm";
var breakFreeAv1854 = "assets/videos/hero/break_free/break_free_av1_854.HN3WTVPV.webm";
var breakFreeH2641280 = "assets/videos/hero/break_free/break_free_h264_1280.P7H5AFXW.mp4";
var breakFreeH2641920 = "assets/videos/hero/break_free/break_free_h264_1920.VD3OTIVQ.mp4";
var breakFreeH2642560 = "assets/videos/hero/break_free/break_free_h264_2560.SBX24NRJ.mp4";
var breakFreeH2643840 = "assets/videos/hero/break_free/break_free_h264_3840.L6D346UN.mp4";
var breakFreeH264426 = "assets/videos/hero/break_free/break_free_h264_426.N7UKN7DL.mp4";
var breakFreeH264640 = "assets/videos/hero/break_free/break_free_h264_640.VS54S5EL.mp4";
var breakFreeH264854 = "assets/videos/hero/break_free/break_free_h264_854.AKGTMYW6.mp4";
var breakFreeVp91280 = "assets/videos/hero/break_free/break_free_vp9_1280.BI7P3W4T.webm";
var breakFreeVp91920 = "assets/videos/hero/break_free/break_free_vp9_1920.UQIEAJZ4.webm";
var breakFreeVp92560 = "assets/videos/hero/break_free/break_free_vp9_2560.FOFH4KAS.webm";
var breakFreeVp93840 = "assets/videos/hero/break_free/break_free_vp9_3840.VKNFLIWB.webm";
var breakFreeVp9426 = "assets/videos/hero/break_free/break_free_vp9_426.CWEZUEQQ.webm";
var breakFreeVp9640 = "assets/videos/hero/break_free/break_free_vp9_640.X5H75VBM.webm";
var breakFreeVp9854 = "assets/videos/hero/break_free/break_free_vp9_854.L7KXRADV.webm";
...
var tokyoShuffleAvif1280 = "assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_1280.ZLQHM4VG.avif";
var tokyoShufflePng1280 = "assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_1280.OYFSUR65.png";
var tokyoShuffleWebp1280 = "assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_1280.EEEVR5AA.webp";
var tokyoShuffleAvif1920 = "assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_1920.AGNLWXP5.avif";
var tokyoShufflePng1920 = "assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_1920.C5GVT334.png";
var tokyoShuffleWebp1920 = "assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_1920.A6XB2PAB.webp";
var tokyoShuffleAvif2560 = "assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_2560.BOHJXWWH.avif";
var tokyoShufflePng2560 = "assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_2560.B7WTBGPV.png";
var tokyoShuffleWebp2560 = "assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_2560.K223UIDG.webp";
var tokyoShuffleAvif3840 = "assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_3840.6YTTSAWO.avif";
var tokyoShufflePng3840 = "assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_3840.KWJHCM5F.png";
var tokyoShuffleWebp3840 = "assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_3840.WPHZ3N2S.webp";
var tokyoShuffleAvif426 = "assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_426.JEFZFOJK.avif";
var tokyoShufflePng426 = "assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_426.36AKL4KN.png";
var tokyoShuffleWebp426 = "assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_426.VQWFGCSR.webp";
var tokyoShuffleAvif640 = "assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_640.KF7GISVX.avif";
var tokyoShufflePng640 = "assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_640.Q7RF6Q5K.png";
var tokyoShuffleWebp640 = "assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_640.3AQWY55B.webp";
var tokyoShuffleAvif854 = "assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_854.R5Q7G7ZO.avif";
var tokyoShufflePng854 = "assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_854.AHAG6T2U.png";
var tokyoShuffleWebp854 = "assets/videos/hero/tokyo_shuffle/posters/tokyo_shuffle_854.OH5ALQCN.webp";
var tokyoShuffleAv11280 = "assets/videos/hero/tokyo_shuffle/tokyo_shuffle_av1_1280.LTL754GX.webm";
var tokyoShuffleAv11920 = "assets/videos/hero/tokyo_shuffle/tokyo_shuffle_av1_1920.HCHG3L6F.webm";
var tokyoShuffleAv12560 = "assets/videos/hero/tokyo_shuffle/tokyo_shuffle_av1_2560.VZLSLY3T.webm";
var tokyoShuffleAv13840 = "assets/videos/hero/tokyo_shuffle/tokyo_shuffle_av1_3840.IRQNSZL3.webm";
var tokyoShuffleAv1426 = "assets/videos/hero/tokyo_shuffle/tokyo_shuffle_av1_426.VLHMAUSF.webm";
var tokyoShuffleAv1640 = "assets/videos/hero/tokyo_shuffle/tokyo_shuffle_av1_640.UJYTIEMI.webm";
var tokyoShuffleAv1854 = "assets/videos/hero/tokyo_shuffle/tokyo_shuffle_av1_854.T4JUCEGM.webm";
var tokyoShuffleH2641280 = "assets/videos/hero/tokyo_shuffle/tokyo_shuffle_h264_1280.T7YZV75R.mp4";
var tokyoShuffleH2641920 = "assets/videos/hero/tokyo_shuffle/tokyo_shuffle_h264_1920.IYJ32LIS.mp4";
var tokyoShuffleH2642560 = "assets/videos/hero/tokyo_shuffle/tokyo_shuffle_h264_2560.EWUECAZU.mp4";
var tokyoShuffleH2643840 = "assets/videos/hero/tokyo_shuffle/tokyo_shuffle_h264_3840.7IMTXTLT.mp4";
var tokyoShuffleH264426 = "assets/videos/hero/tokyo_shuffle/tokyo_shuffle_h264_426.L7AX42QP.mp4";
var tokyoShuffleH264640 = "assets/videos/hero/tokyo_shuffle/tokyo_shuffle_h264_640.VLIHCIGJ.mp4";
var tokyoShuffleH264854 = "assets/videos/hero/tokyo_shuffle/tokyo_shuffle_h264_854.4V5TCTNN.mp4";
var tokyoShuffleVp91280 = "assets/videos/hero/tokyo_shuffle/tokyo_shuffle_vp9_1280.OZ63FAA6.webm";
var tokyoShuffleVp91920 = "assets/videos/hero/tokyo_shuffle/tokyo_shuffle_vp9_1920.BL4HBHVE.webm";
var tokyoShuffleVp92560 = "assets/videos/hero/tokyo_shuffle/tokyo_shuffle_vp9_2560.HQFL3XZA.webm";
var tokyoShuffleVp93840 = "assets/videos/hero/tokyo_shuffle/tokyo_shuffle_vp9_3840.AMTWROJH.webm";
var tokyoShuffleVp9426 = "assets/videos/hero/tokyo_shuffle/tokyo_shuffle_vp9_426.MROBMCDW.webm";
var tokyoShuffleVp9640 = "assets/videos/hero/tokyo_shuffle/tokyo_shuffle_vp9_640.YMSOWHCY.webm";
var tokyoShuffleVp9854 = "assets/videos/hero/tokyo_shuffle/tokyo_shuffle_vp9_854.MXKV5CQO.webm";
var origin, toUrl, plainLogo, rawHeroVideos;
var init_data = __esm({
  "src/assets/javascripts/hero/video/data.ts"() {
    "use strict";
    init_types5();
    origin = window.location.origin || import.meta.url.replace("docs", "");
    toUrl = (pathname) => {
      pathname = pathname.replace("~", "").replace("..", "assets");
      return new URL(pathname, origin);
    };
    plainLogo = toUrl(svgLogo);
    rawHeroVideos = [
      {
        variants: {
          av1: {
...
function filterActiveClass(classList, baseClass) {
  ...
  Array.from(classList).filter((c) => c in classObj).forEach((c) => {
    if (c === active) {
  ...
  return Object.fromEntries(Object.entries(classObj).map(([k, v]) => v !== "" && k ? [k, v] : [k, null]));
}
...
var getAv1MediaType, getH264MediaType, getVp9MediaType;
var init_utils3 = __esm({
  "src/assets/javascripts/hero/video/utils.ts"() {
    "use strict";
    init_data();
    init_utils();
    getAv1MediaType = (width) => {
      const seqlevelMap = {
        426: "16",
        640: "16",
        854: "16",
    ...
    getH264MediaType = () => "video/mp4";
    getVp9MediaType = (width) => {
      const levelMap = {
        426: "20",
        640: "21",
        854: "30",
    ...
    VideoElement = class {
      constructor(heroVideo, properties) {
        ...
        for (const prop of Object.keys(this.properties)) {
          ...
          try {
            logger.debug("Setting property ".concat(key, " to ").concat(this.properties[key], " for video element"));
...
var customWindow2, document$2, VideoManager;
var init_videoManager = __esm({
  "src/assets/javascripts/hero/video/videoManager.ts"() {
    "use strict";
    init_gsap();
    init_esm5();
    ...
    init_videoElement();
    init_data();
    customWindow2 = window;
    ({ document$: document$2 } = customWindow2);
    VideoManager = class _VideoManager {
      constructor() {
        this.store = HeroStore.getInstance();
        this.container = document.querySelector(".hero.first .media__container") || (() => {
          const div = document.createElement("div");
          div.classList.add("media__container");
          const parent = document.querySelector(".hero.first .hero__bg");
          parent == null ? void 0 : parent.append(div);
          return div;
        })();
        ...
        this.logo = (() => {
          ...
          div.appendChild(img);
          gsapWithCSS.set(div, { autoAlpha: 0, scale: 0, yPercent: 5 });
          logger.debug("Logo created and appended:", img);
          return div;
        })();
        this.timeline = gsapWithCSS.timeline();
        this.textAnimationTimeline = gsapWithCSS.timeline();
        this.subscriptions = new Subscription();
      ...
      init_subscriptions() {
        ...
        const video$ = videoState$.pipe(distinctUntilKeyChanged("canPlay"), tap((canPlay) => {
        ...
        const stallHandler$ = combineLatest([
          ...
          Promise.resolve(setTimeout(() => {
            if (this.element.currentTime > 0 && this.element.paused === false && !this.element.ended) {
              return;
            } else {
              this.pause();
        ...
        const elementChildren = gsapWithCSS.utils.toArray(this.element.children);
        ...
        const metadata$ = fromEvent(this.element, "loadedmetadata").pipe(filter((ev) => {
        ...
        const canplaythrough$ = fromEvent(this.element, "canplaythrough").pipe(filter((ev) => {
          return ev.target instanceof HTMLMediaElement;
        }), distinctUntilChanged(), tap(() => {
          this.status = "loaded";
        ...
        const playing$ = fromEvent(this.element, "playing").pipe(filter((ev) => {
          return ev.target instanceof HTMLMediaElement;
        }), distinctUntilChanged(), tap(() => {
          this.status = "playing";
          this.timeline.play();
        ...
        const textAnimator$ = fromEvent(this.element, "timeupdate").pipe(map((ev) => {
          return ev instanceof Event && ev.target instanceof HTMLMediaElement ? ev.target : null;
        }), filter((el) => el !== null), map((target) => {
        ...
        }), distinctUntilKeyChanged("canAnimate"), switchMap(({ canAnimate, isAnimating, isPlaying }) => {
          if (canAnimate) {
        ...
        const waiting$ = fromEvent(this.element, "waiting").pipe(filter((ev) => {
          return ev.target instanceof HTMLMediaElement;
        }), distinctUntilChanged(), tap(() => {
          this.pause();
        }));
      ...
      initVideo() {
        var _a2;
        gsapWithCSS.set(this.container, { contentVisibility: "hidden" });
        if (this.videoStore.length === 0) {
          ...
          if (this.videoStore.length > 1 && ((_a2 = this.video) == null ? void 0 : _a2.heroVideo.baseName) !== BACKUP_PICTURE) {
            this.backupPicture = new VideoElement(randomized[1]).picture;
          } else {
            this.backupPicture = this.video.picture.cloneNode(true);
          }
      ...
      constructTextTimeline() {
        const ctaHeaders = gsapWithCSS.utils.toArray(this.ctaContainer.querySelectorAll("h1, h2"));
        this.textAnimationTimeline = gsapWithCSS.timeline({
          ...
          onComplete: () => {
            gsapWithCSS.to(this.element, { autoAlpha: 1, duration: 0.5 });
            gsapWithCSS.to(ctaHeaders, { autoAlpha: 1, duration: 0.5 });
            this.status = "paused";
            Promise.resolve(() => {
              this.stop();
              this.play();
      ...
      constructTimeline() {
        this.timeline = gsapWithCSS.timeline({
          ...
          repeat: 0,
          // we handle this manually to control the video
          ease: "none"
        }).add([
          "startVideo",
          () => {
        ...
        ], "<").add(gsapWithCSS.set(this.container, { autoAlpha: 1 }));
      }
      setEmphasisAnimations() {
        ...
        return gsapWithCSS.timeline().add([
          "subtleEmphasis",
          gsapWithCSS.timeline()["emphasize"](subtleTargets, SUBTLE_EMPHASIS_CONFIG),
          ">"
        ], ">").add([
          "strongEmphasis",
          gsapWithCSS.timeline()["emphasize"](strongTargets, STRONG_EMPHASIS_CONFIG),
          ">"
        ], ">=0.5");
      }
      loadVideo() {
      ...
      loadBackup() {
        const backup = this.poster.classList.contains("hero__poster--active") ? this.poster : this.backupPicture;
      ...
      initiateFallback() {
        ...
        }).set(this.container, { autoAlpha: 1 }).set(this.ctaContainer, { autoAlpha: 1 });
      ...
      animateText(overrideVars = {}) {
        return gsapWithCSS.timeline().call(() => {
          if (this.element.classList.contains("hero__video--active")) {
            toggleActiveClass(this.element, "hero__video", false);
          }
        }).set(".text-animation__container", { autoAlpha: 1 })["animateMessage"](".text-animation__container", {
...
// src/assets/javascripts/hero/video/index.ts
...
// src/assets/javascripts/hero/index.ts
var init_hero = __esm({
  "src/assets/javascripts/hero/index.ts"() {
    "use strict";
    init_animations();
...
var customWindow3, weAreDev, viewport$4, initialUrl, isFullyVisible, isTransitioning, noVideo, videoPredicate, predicates, HeroStore;
var init_store = __esm({
  "src/assets/javascripts/state/store.ts"() {
    ...
    init_utils();
    init_types3();
    customWindow3 = window;
    weAreDev = isDev(new URL(customWindow3.location.href));
    ({ viewport$: viewport$4 } = customWindow3);
    initialUrl = new URL(customWindow3.location.href);
    isFullyVisible = (state) => state.atHome && state.landingVisible && state.pageVisible;
    isTransitioning = (state) => state.isTransitioning;
    noVideo = (state) => state.prefersReducedMotion;
    videoPredicate = (state) => ({
    ...
    HeroStore = class _HeroStore {
      ...
      constructor() {
        ...
        this.transitionState$ = new BehaviorSubject({ isTransitioning: true });
      ...
      initSubscriptions() {
        ...
        ]).pipe(map(([landingVisible, _currentSection]) => {
          return landingVisible;
        }), distinctUntilChanged(), shareReplay(1));
        const landingVisible$ = atHome$.pipe(filter((atHome) => atHome), switchMap(() => landing$), filter((landingVisible) => !!landingVisible), tap(this.createObserver("landingVisible$", (landingVisible) => ({ landingVisible }))));
        ...
        const tab$ = watchTabs(getComponentElement("tabs"), { viewport$: viewport$4, header$: header$3 });
        const headerWatch$ = combineLatest([header$3, tab$]).pipe(tap(([header2, tabs]) => {
          if (tabs.hidden) {
            ...
            setCssVariable("--header-height", header2.hidden ? "".concat(tabHeight, "px") : "".concat(header2.height + tabHeight, "px"));
          }
        }), map(([header2, _tabs]) => header2), tap(this.createObserver("header$", (header2) => ({ header: header2 }))));
        ...
        ]).pipe(map(([viewport, header2, portrait]) => {
          return {
        ...
        const loc$ = navigationEvents$.pipe(tap(this.createObserver("location$", (location2) => ({
          location: location2
        }))));
        const section$ = this.section$.pipe(tap(this.createObserver("section$", (currentSection) => ({
          currentSection
        }))), distinctUntilChanged(), shareReplay(1));
        this.subscriptions.add(atHome$.subscribe());
      ...
      getStateObservable(observerFunc, key, observableName, predicate) {
        ...
        })), distinctUntilKeyChanged(key), shareReplay(1), tap(this.getComponentObserver(observableName, observerFunc)));
      }
...
// src/assets/javascripts/state/index.ts
...
async function locateMissingWorker() {
  ...
  if (response.ok) {
    ...
    const cacheWorker = meta.find(([key, _value]) => key.includes("cacheWorker") && !key.includes(".map"));
    if (!cacheWorker) {
...
var cacheWorkerUrl = "cacheWorker.4OWGWVKB.js";
if ("serviceWorker" in navigator && window.isSecureContext) {
  logger.debug("Registering service worker");
  const register2 = async () => {
    navigator.serviceWorker.register(cacheWorkerUrl, { scope: "/" }).catch(async (error) => {
...
if (imgLogo && imgLogo instanceof HTMLElement && supportsHasSelector() && imgLogo.hasAttribute("hidden")) {
...
var nav$ = of(setNavId());
var heroTextSize$ = async function() {
...
var pageConfigs = [
  {
    matcher: isHome,
    location: "home",
    observables: [nav$, color$, from(heroTextSize$), observer$4, videoManager$]
  },
  {
    matcher: isLicense,
    location: "licenses",
...
var pageSubscription$ = navigationEvents$.pipe(map((url2) => {
  ...
  const allObservables = configs.flatMap((config6) => config6.observables.map((obs) => obs.pipe(tap(() => logger.info("Running observables for ".concat(config6.location))), catchError((error) => {
...
gsap/ScrollToPlugin.js:
  ...
  *)
*/
//# sourceMappingURL=index.HAKUYMYK.js.map
