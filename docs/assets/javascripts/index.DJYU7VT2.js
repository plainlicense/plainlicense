...
var VideoManager = class _VideoManager {
  ...
  initSubscriptions() {
    ...
    const statusSetter$ = state$.pipe(map(({ canPlay, prefersReducedMotion, currentSection }) => ({
      ...
      prefersReducedMotion,
      currentSection
    })), tap(this.handleReducedMotion.bind(this)), distinctUntilChanged((prev, cur) => prev.canPlay === cur.canPlay && prev.currentSection === cur.currentSection), debounceTime(100), tap(({ prefersReducedMotion, currentSection, canPlay }) => {
    ...
    const play$ = canplaythrough$.pipe(filter(() => this.canPlay === true && this.element.readyState === 4 && !this.isPlaying()), exhaustMap(() => {
      ...
      next: (canPlay) => {
        ...
        if (canPlay) {
          ...
          secondaryObservables.forEach((obs) => {
            if (obs !== motionSub$) {
              logger.debug("Subscribing to secondary observable: ".concat(obs));
  ...
  determineVideoStatus(canPlay, currentSection) {
    ...
    if (canPlay) {
      if (!this.element.duration) {
        ...
        if (this.isPlaying()) {
          return "playing";
        } else if (currentSection >= 1 /* Impact */ || this.element.paused) {
      ...
      if (currentSection >= 1 /* Impact */ && this.status !== "not_initialized" || this.hasPlayed || this.element.paused) {
        return this.element.currentTime > 0 || this.hasPlayed ? "paused" : "loaded";
...
gsap/ScrollToPlugin.js:
  ...
  *)
*/
//# sourceMappingURL=index.DJYU7VT2.js.map
