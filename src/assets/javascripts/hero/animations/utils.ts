...
export function normalizeResolution(): number {
  ...
  if (
    ...
    !('offset' in viewport) ||
    typeof (viewport as Viewport).offset !== 'object' ||
    typeof (viewport as Viewport).offset.x !== 'number' ||
    typeof (viewport as Viewport).offset.y !== 'number'
  ) {
    throw new Error('Viewport or viewport offset is not defined or has invalid structure');
  }
  const { offset } = viewport as { offset: { x: number; y: number } };
  const resolution = Math.max(offset.y, offset.x);
 ...
 */
function tryObject(obj: unknown): Element[] | null {
  if (obj === null || typeof obj !== 'object' || (Array.isArray(obj) && obj.length === 0)) {
    return [];
  }
