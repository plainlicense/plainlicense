      ...
      if (entry.isDirectory()) {
        ...
        if (relativePath.startsWith('licenses/')) {
          if (content.trim().length < 100) {
            console.error(`[VALIDATION ERROR] License ${relativePath} body too short: ${content.length} chars (min 100)`);
            errors++;
          }
        } else if (relativePath.startsWith('blog/')) {
          if (content.trim().length < 200) {
            console.error(`[VALIDATION ERROR] Blog post ${relativePath} body too short: ${content.length} chars (min 200)`);
            errors++;
          }
        }
      }
