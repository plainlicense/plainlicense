  ...
  it("mapping files reference valid IDs in markdown content", async () => {
    ...
    for (const file of mappingFiles) {
      const mappingData = JSON.parse(
        await fs.readFile(path.join(mappingsDir, file), "utf8"),
      );
      ...
      for (const cat of categories) {
        ...
        const mdFile = mdFiles.find(
          (f) => f.toLowerCase() === `${spdxId.toLowerCase()}.md`,
        );

        if (mdFile) {
          found = true;
          const mdContent = await fs.readFile(
            path.join(catPath, mdFile),
            "utf8",
          );

          for (const m of mappingData.mappings) {
            const plainClauses =
              m.plain_clauses || (m.plain_clause ? [m.plain_clause] : []);
            const originalClauses =
              m.original_clauses ||
              (m.original_clause ? [m.original_clause] : []);
