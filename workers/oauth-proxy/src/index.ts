...
export interface Env {
  GITHUB_CLIENT_ID: Promise<{ get(): string }>;
  GITHUB_CLIENT_SECRET: Promise<{ get(): string }>;
  ALLOWED_ORIGINS: string;
  JWT_SECRET: Promise<{ get(): string }>;
}

type JWTPayload = {
  sub: string;
  gh: string; // GitHub access token
  refresh?: boolean; // Indicates if this is a refresh token
...
export default {
  ...
  async handleAuth(url: URL, env: Env): Promise<Response> {
    ...
    redirectUri.searchParams.set(
      "client_id",
      await env.GITHUB_CLIENT_ID.then((c) => c.get()),
    );
  ...
  async handleCallback(url: URL, env: Env): Promise<Response> {
    ...
    const tokenResponse = await fetch(
      "https://github.com/login/oauth/access_token",
      {
        ...
        body: JSON.stringify({
          client_id: await env.GITHUB_CLIENT_ID.then((c) => c.get()),
          client_secret: await env.GITHUB_CLIENT_SECRET.then((c) => c.get()),
          code,
        }),
      },
    );
    ...
    const accessToken = await this.createJWT(
      { sub: "user", gh: data.access_token } as JWTPayload,
      await env.JWT_SECRET.then((c) => c.get()),
      15 * 60,
    );
    const refreshToken = await this.createJWT(
      { sub: "user", gh: data.access_token, refresh: true } as JWTPayload,
      await env.JWT_SECRET.then((c) => c.get()),
      7 * 24 * 60 * 60,
    );

    // Pass state back so the opener can verify CSRF protection client-side
      ...
      <html>
        <body>
          <script>
            ...
            const state = "${state}";
            // Send back to opener — client verifies state matches what it sent
  ...
  async handleRefresh(request: Request, env: Env): Promise<Response> {
    ...
    const newAccessToken = await this.createJWT(
      { sub: "user", gh: payload.gh } as JWTPayload,
      await env.JWT_SECRET.then((c) => c.get()),
      15 * 60,
    );
    // Optional: Rotate refresh token too (refresh-the-refresh)
    const newRefreshToken = await this.createJWT(
      { sub: "user", gh: payload.gh, refresh: true } as JWTPayload,
      await env.JWT_SECRET.then((c) => c.get()),
      7 * 24 * 60 * 60,
    );

    return new Response(
      JSON.stringify({ token: newAccessToken, refreshToken: newRefreshToken }),
      {
